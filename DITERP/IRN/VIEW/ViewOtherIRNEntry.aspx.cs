...
public partial class IRN_VIEW_ViewOtherIRNEntry : System.Web.UI.Page
{
    #region " Var "
    ...
    private void LoadData()
    {
        try
        {
            ...
            dt = CommonClasses.Execute("SELECT DISTINCT TOP 500 IRN_CODE, IRN_NO,convert(varchar, IRN_DATE,106) AS IRN_DATE FROM IRN_ENTRY INNER JOIN IRN_DETAIL ON IRN_CODE = IRND_IRN_CODE WHERE IRN_TYPE=1 AND  IRN_TRANS_TYPE=0 AND  (IRN_ENTRY.ES_DELETE = 0) AND IRN_CM_ID ='" + Session["CompanyId"].ToString() + "' ORDER BY IRN_NO DESC");
            if (dt.Rows.Count > 0)
            {
        ...
        try
        {
            if (e.CommandName.Equals("View"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For View"))
                {
                    ...
                    Response.Redirect("~/IRN/ADD/OtherIRNEntry.aspx?c_name=" + type + "&i_uom_code=" + RSM_CODE, false);
                }
                else
                {
                    PanelMsg.Visible = true;
    ...
    protected void btnAddNew_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string type = "INSERT";
                Response.Redirect("~/IRN/ADD/OtherIRNEntry.aspx?c_name=" + type, false);
            }
            else
            {
                PanelMsg.Visible = true;
