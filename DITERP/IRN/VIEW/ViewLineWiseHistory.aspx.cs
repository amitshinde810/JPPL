...
public partial class IRN_VIEW_ViewLineWiseHistory : System.Web.UI.Page
{
    static string right = "";
    ...
    private void LoadCombos()
    {
        try
        {
            ...
            DataTable dtStage = CommonClasses.Execute("SELECT DISTINCT LM_CODE AS STG_CODE,LM_NAME AS STG_NAME FROM LINE_MASTER INNER JOIN  LINE_CHANGE ON LINE_MASTER.LM_CODE = LINE_CHANGE.LC_LM_CODE INNER JOIN  ITEM_MASTER ON LINE_CHANGE.LC_I_CODE = ITEM_MASTER.I_CODE WHERE     (LINE_MASTER.ES_DELETE = 0)  AND LC_DATE BETWEEN '" + Convert.ToDateTime(dateFrom).ToString("dd/MMM/yyyy") + "' AND '" + Convert.ToDateTime(LastDate).ToString("dd/MMM/yyyy") + "' AND LM_CM_ID = '" + (string)Session["CompanyId"] + "' ORDER BY STG_NAME  ");
            ddlStage.DataSource = dtStage;
            ddlStage.DataTextField = "STG_NAME";
            ddlStage.DataValueField = "STG_CODE";
            ddlStage.DataBind();
            ddlStage.Items.Insert(0, new ListItem("Select Line", "0"));
        }
        catch (Exception Ex)
        {
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                ...
                Response.Redirect("../../IRN/ADD/LineWiseHistory.aspx?Title=" + Title + "&DATE=" + firstDayOfMonth + "&TDATE=" + lastDayOfMonth + "&i_name=" + ddlFinishedComponent.SelectedValue.ToString() + "&L_name=" + ddlStage.SelectedValue.ToString() + "", false);
            }
            else
            {
