...
public partial class IRN_VIEW_ViewStageWiseHistory : System.Web.UI.Page
{
    static string right = "";
    ...
    private void LoadCombos()
    {
        try
        {
            ...
            dtItem = CommonClasses.Execute("SELECT  dISTINCT I_CODE, I_CODENO + ' - '+ I_NAME  AS ICODE_INAME FROM STGAEWISE_ITEM,STAGE_MASTER,ITEM_MASTER where SI_STG_CODE=STG_CODE AND SI_I_CODE=I_CODE AND SI_DATE BETWEEN '" + Convert.ToDateTime(dateFrom).ToString("dd/MMM/yyyy") + "' AND '" + Convert.ToDateTime(LastDate).ToString("dd/MMM/yyyy") + "'  AND STG_CM_ID = '" + (string)Session["CompanyId"] + "' ORDER BY ICODE_INAME   ");
            ...
            DataTable dtStage = CommonClasses.Execute("SELECT  DISTINCT   STG_CODE,STG_NAME FROM STGAEWISE_ITEM,STAGE_MASTER,ITEM_MASTER where SI_STG_CODE=STG_CODE AND SI_I_CODE=I_CODE AND SI_DATE BETWEEN '" + Convert.ToDateTime(dateFrom).ToString("dd/MMM/yyyy") + "' AND '" + Convert.ToDateTime(LastDate).ToString("dd/MMM/yyyy") + "' AND STG_CM_ID = '" + (string)Session["CompanyId"] + "' ORDER BY STG_NAME  ");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                ...
                Response.Redirect("../../IRN/ADD/StageWiseHistory.aspx?Title=" + Title + "&DATE=" + firstDayOfMonth + "&TDATE=" + lastDayOfMonth + "&i_name=" + ddlFinishedComponent.SelectedValue.ToString() + "&L_name=" + ddlStage.SelectedValue.ToString() + "", false);
            }
            else
            {
    ...
    #region chkStage_CheckedChanged
    protected void chkStage_CheckedChanged(object sender, EventArgs e)
    {
    ...
    #endregion chkStage_CheckedChanged
}
