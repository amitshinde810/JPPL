...
public partial class IRN_VIEW_ViewDefectWiseMechiningRejYearly : System.Web.UI.Page
{
    static string right = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='137'");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                ...
                Response.Redirect("../../IRN/ADD/DefectWiseMechiningRejYearly.aspx?Title=" + Title + "&FDATE=" + txtFromDate.Text + "&TDATE=" + Convert.ToDateTime(txtTodate.Text).AddMonths(1).AddDays(-1).ToString("dd/MMM/yyyy") + "&i_name=" + ddlFinishedComponent.SelectedValue.ToString() + "&type=" + tbType.SelectedValue.ToString() + "", false);
            }
            else
            {
