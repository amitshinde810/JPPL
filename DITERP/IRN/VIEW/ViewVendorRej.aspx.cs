...
public partial class IRN_VIEW_ViewVendorRej : System.Web.UI.Page
{
    static string right = "";
    ...
    private void LoadCustomer()
    {
        try
        {
            ...
            {
                dtVendor = CommonClasses.Execute("SELECT DISTINCT P_CODE, PARTY_MASTER.P_NAME FROM PARTY_MASTER,IRN_ENTRY,IRN_DETAIL WHERE IRND_IRN_CODE = IRN_CODE AND IRN_ENTRY.ES_DELETE = 0 AND IRND_P_CODE = P_CODE AND IRN_CM_ID = '" + Session["CompanyId"].ToString() + "' ORDER BY P_NAME ");
            }
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                ...
                Response.Redirect("../../IRN/ADD/VendorRejYearly.aspx?Title=" + Title + "&FDATE=" + Convert.ToDateTime(txtFromDate.Text).ToString("dd/MMM/yyyy") + "&TDATE=" + Convert.ToDateTime(txtTodate.Text).AddMonths(1).AddDays(-1).ToString("dd/MMM/yyyy") + "&type=" + rbType.SelectedValue.ToString() + "&Cond=" + StrCond + "&StrParty=" + StrParty + "", false);
            }
            else
            {
    ...
    public bool ShowMessage(string DiveName, string Message, string MessageType)
    {
        ...
        {
            CommonClasses.SendError("Vendor Wise Rejection Yearly", "ShowMessage", Ex.Message);
            return false;
        }
    }
    #endregion
