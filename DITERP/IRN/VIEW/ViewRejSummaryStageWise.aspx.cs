...
public partial class IRN_VIEW_ViewRejSummaryStageWise : System.Web.UI.Page
{
    static string right = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='146'");
    ...
    private void LoadCombos()
    {
        try
        {
            ...
            {
                dtItemDet = CommonClasses.Execute("SELECT DISTINCT SI_STG_CODE ,STG_NAME FROM STGAEWISE_ITEM,STAGE_MASTER  where SI_ACTIVE=1 AND SI_STG_CODE=STG_CODE AND ES_DELETE=0 ORDER BY STG_NAME");
            }
            else
            {
                dtItemDet = CommonClasses.Execute("SELECT DISTINCT SI_STG_CODE ,STG_NAME FROM STGAEWISE_ITEM,STAGE_MASTER  where SI_ACTIVE=1 AND SI_STG_CODE=STG_CODE AND ES_DELETE=0  AND   SI_DATE BETWEEN '" + Convert.ToDateTime(txtFromDate.Text).ToString("dd/MMM/yyyy") + "'   AND '" + Convert.ToDateTime(txtTodate.Text).ToString("dd/MMM/yyyy") + "' ORDER BY STG_NAME");
            }

            ddlFinishedComponent.DataSource = dtItemDet;
            ddlFinishedComponent.DataTextField = "STG_NAME";
            ddlFinishedComponent.DataValueField = "SI_STG_CODE";
            ddlFinishedComponent.DataBind();
            ddlFinishedComponent.Items.Insert(0, new ListItem("Select Stage", "0"));

            ddlItemCode.DataSource = dtItemDet;
            ddlItemCode.DataTextField = "STG_NAME";
            ddlItemCode.DataValueField = "SI_STG_CODE";
            ...
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                if (chkAllItem.Checked == false)
                {
                    if (ddlFinishedComponent.SelectedIndex == 0)
                    {
                        ...
                        lblmsg.Text = "Please Select Stage Name";
                ...
                Response.Redirect("../../IRN/ADD/RejSummaryStageWise.aspx?Title=" + Title + "&FDATE=" + Convert.ToDateTime(txtFromDate.Text).ToString("dd/MMM/yyyy") + "&TDATE=" + Convert.ToDateTime(txtTodate.Text).AddMonths(1).AddDays(-1).ToString("dd/MMM/yyyy") + "&i_name=" + ddlFinishedComponent.SelectedValue.ToString() + "&type=" + tbType.SelectedValue.ToString() + "&Ptype=" + rbType.SelectedValue.ToString() + "", false);
            }
            else
            {
