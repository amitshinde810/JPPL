...
public partial class IRN_ADD_CustRej : System.Web.UI.Page
{
    #region Variable
    ...
    private void LoadIName()
    {
        ...
        {
            if (txtInvoiceNo.Text != "" && txtInvoiceNo.Text != "0")
                dt = CommonClasses.Execute("select DISTINCT I_CODE,I_NAME from INVOICE_DETAIL,ITEM_MASTER,INVOICE_MASTER where INM_NO=" + txtInvoiceNo.Text.Trim() + " AND  INM_P_CODE=" + ddlCustomer.SelectedValue + "  and IND_INM_CODE=INM_CODE AND INVOICE_MASTER.ES_DELETE=0 AND INM_TYPE='TAXINV'  and I_CODE=IND_I_CODE");
            else
                dt = CommonClasses.Execute("select DISTINCT I_CODE,I_NAME from CUSTPO_MASTER,CUSTPO_DETAIL,ITEM_MASTER where CPOM_P_CODE=" + ddlCustomer.SelectedValue + " AND CPOM_CODE=CPOD_CPOM_CODE     AND CPOD_I_CODE=I_CODE");
    ...
    bool SaveRec()
    {
        ...
        try
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                int ginNo = 0;
                DataTable dt = new DataTable();
                dt = CommonClasses.Execute("Select isnull(max(CR_GIN_NO),0) as CR_GIN_NO FROM CUSTREJECTION_MASTER WHERE CR_CM_CODE = " + (string)Session["CompanyCode"] + "  AND ISNULL(CR_TRANS_TYPE,0)=1 and ES_DELETE=0");
                if (dt.Rows.Count > 0)
                {
