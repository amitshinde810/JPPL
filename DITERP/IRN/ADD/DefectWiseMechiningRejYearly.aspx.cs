...
public partial class IRN_ADD_DefectWiseMechiningRejYearly : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string From, string Todt, string i_name, string Type)
    {
        try
        {
            ...
            if (dtNew.Rows.Count > 0)
            {
                ...
                if (Type == "0")
                {
                    rptname.SetParameterValue("txtPeriod", "COMPONENT SPECIFIC DEFECT SPECIFIC MONTH WISE MACHINING REJECTION DETAILS From " + Convert.ToDateTime(From).ToString("MMM yyyy") + " To " + Convert.ToDateTime(Todt).ToString("MMM yyyy"));
                }
                if (Type == "1")
                {
                    rptname.SetParameterValue("txtPeriod", "COMPONENT SPECIFIC DEFECT SPECIFIC MONTH WISE MACHINING REJECTION DETAILS From " + Convert.ToDateTime(From).ToString("MMM yyyy") + " To " + Convert.ToDateTime(Todt).ToString("MMM yyyy"));
