...
public partial class RoportForms_VIEW_DCTReturnRegister : System.Web.UI.Page
{
    static string right = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='265'");
    ...
    private void LoadCombos()
    {
        try
        {
            ...
            dtItemDet = CommonClasses.Execute("select distinct I_CODE,I_CODENO,I_NAME FROM ITEM_MASTER,DC_RETURN_DETAIL WHERE I_CM_COMP_ID=" + Session["CompanyId"].ToString() + " and   DND_I_CODE=I_CODE  ORDER BY I_NAME ");
        ...
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For Menu"))
            {
               ...
               if (chkAllCustomer.Checked == false)
                ...
                if (chkDateAll.Checked != true)
                {
                    strcondition = strcondition + "  DNM_DATE  BETWEEN '" + Convert.ToDateTime(txtFromDate.Text).ToString("dd/MMM/yyyy") + "'  AND  '" + Convert.ToDateTime(txtToDate.Text).ToString("dd/MMM/yyyy") + "' AND ";
                }
                else
                {
                    strcondition = strcondition + "  DNM_DATE  BETWEEN '" + Convert.ToDateTime(Session["OpeningDate"]).ToString("dd/MMM/yyyy") + "'  AND  '" + Convert.ToDateTime(Session["ClosingDate"]).ToString("dd/MMM/yyyy") + "' AND ";
                }
                if (chkAllCustomer.Checked != true)
                {
                    strcondition = strcondition + "  DNM_P_CODE ='" + ddlCustomerName.SelectedValue + "'  AND ";
                }
                if (chkAllItem.Checked != true)
                {
                    strcondition = strcondition + "  DND_I_CODE ='" + ddlFinishedComponent.SelectedValue + "'  AND ";
                }

                Response.Redirect("../../RoportForms/ADD/DCTRetrunRegister.aspx?Title=" + Title + "&FromDate=" + From + "&ToDate=" + To + "&datewise=" + str + "&detail=" + str1 + "&Cond=" + strcondition + "&Type=" + ddlUser.SelectedValue + "", false);
            }
            else
            {
                PanelMsg.Visible = true;
