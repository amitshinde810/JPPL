...
public partial class RoportForms_VIEW_ViewSalesMISWeightWise : System.Web.UI.Page
{
    static string right = "";
    ...
    private void LoadItems()
    {
        try
        {
            ...
            dtItemDet = CommonClasses.Execute("SELECT distinct ITEM_MASTER.I_CODENO,ITEM_MASTER.I_NAME,ITEM_MASTER.I_CODE FROM INVOICE_MASTER INNER JOIN INVOICE_DETAIL ON INVOICE_MASTER.INM_CODE = INVOICE_DETAIL.IND_INM_CODE INNER JOIN ITEM_MASTER ON INVOICE_DETAIL.IND_I_CODE = ITEM_MASTER.I_CODE WHERE " + str + " (ITEM_MASTER.I_CM_COMP_ID = " + (string)Session["CompanyId"] + ") AND (ITEM_MASTER.ES_DELETE = 0) and (INVOICE_MASTER.ES_DELETE=0) order by I_CODENO");
            ...
            dtItemDet.DefaultView.Sort = "I_NAME";
            dtItemDet.DefaultView.ToTable();
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(5, 1)), this, "For Print"))
            {
                ...
                Response.Redirect("~/RoportForms/ADD/SalesMISWeightWise.aspx?Title=" + Title + "&FromDate=" + From + "&ToDate=" + To + "&datewise=" + str + "&detail=" + str1 + "&Cond=" + strCondition + "", false);
            }
        }
        catch (Exception Ex)
        {
    ...
    protected void ddlCustomerName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable dtItemDet = new DataTable();
            dtItemDet = CommonClasses.Execute("SELECT distinct ITEM_MASTER.I_CODENO,ITEM_MASTER.I_NAME,ITEM_MASTER.I_CODE FROM INVOICE_MASTER INNER JOIN INVOICE_DETAIL ON INVOICE_MASTER.INM_CODE = INVOICE_DETAIL.IND_INM_CODE INNER JOIN ITEM_MASTER ON INVOICE_DETAIL.IND_I_CODE = ITEM_MASTER.I_CODE WHERE " + str + "  INM_P_CODE ='" + ddlCustomerName.SelectedValue + "' AND (ITEM_MASTER.I_CM_COMP_ID = " + (string)Session["CompanyId"] + ") AND (ITEM_MASTER.ES_DELETE = 0) and (INVOICE_MASTER.ES_DELETE=0) order by I_NAME");
        ...
        {
            CommonClasses.SendError("Tax Invoice Register", "ddlCustomerName_SelectedIndexChanged", Ex.Message);
        }
    }
}
