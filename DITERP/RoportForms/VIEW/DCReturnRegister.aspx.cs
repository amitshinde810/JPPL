...
public partial class RoportForms_VIEW_DCReturnRegister : System.Web.UI.Page
{
    static string right = "";
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='264'");
    ...
    private void LoadCombos()
    {
        try
        {
            ...
            dtItemDet = CommonClasses.Execute("select distinct I_CODE,I_CODENO+' - '+I_NAME AS I_NAME FROM ITEM_MASTER,DC_RETURN_DETAIL WHERE I_CM_COMP_ID=" + Session["CompanyId"].ToString() + " and   DND_I_CODE=I_CODE  ORDER BY I_NAME ");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For Menu"))
            {
                ...
                Response.Redirect("../../RoportForms/ADD/DCRetrunRegister.aspx?Title=" + Title + "&FromDate=" + From + "&ToDate=" + To + "&datewise=" + str + "&detail=" + str1 + "&Cond=" + strcondition + "&Type=" + ddlUser.SelectedValue + "", false);
            }
            else
            {
                PanelMsg.Visible = true;
