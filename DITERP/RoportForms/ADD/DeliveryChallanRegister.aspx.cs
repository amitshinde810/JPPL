...
public partial class RoportForms_ADD_DeliveryChallanRegister : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string From, string To, string group, string way, string Condition)
    {
        try
        {
            ...
            Query = "Select DCM_CODE,DCM_DATE,DCM_NO,I_CODENO,I_NAME,P_NAME,I_UOM_NAME as I_UOM_NAME,DCM_TYPE,DCD_ORD_QTY from DELIVERY_CHALLAN_MASTER,DELIVERY_CHALLAN_DETAIL,ITEM_MASTER,PARTY_MASTER,ITEM_UNIT_MASTER where   " + Condition + "    DCM_TYPE='DLC' AND  DCM_CODE=DCD_DCM_CODE and DCD_I_CODE=I_CODE and DCM_P_CODE=P_CODE and DELIVERY_CHALLAN_MASTER.ES_DELETE=0 and DCD_UM_CODE=ITEM_UNIT_MASTER.I_UOM_CODE";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (group == "Datewise")
                {
                    rptname.Load(Server.MapPath("~/Reports/rptDeliverychallanRegisterDateWise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptDeliverychallanRegisterDateWise.rpt");
                    rptname.Refresh();
                ...
                if (group == "SuppWise")
                {
                    rptname.Load(Server.MapPath("~/Reports/rptDeliverychallanRegisterSuppWise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptDeliverychallanRegisterSuppWise.rpt");
                    rptname.Refresh();
                    rptname.SetDataSource(dt);
                    rptname.SetParameterValue("txtCompName", Session["CompanyName"].ToString());
