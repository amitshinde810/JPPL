...
public partial class RoportForms_ADD_SubContractorStockLedger : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string Condition, string From, string To, string ttype, string type, string Party)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            rptname.Load(Server.MapPath("~/Reports/rptVendorStockBOMDetails1.rpt"));
            rptname.FileName = Server.MapPath("~/Reports/rptVendorStockBOMDetails1.rpt");
            rptname.Refresh();
    ...
    private void GenerateReport1(string Title, string Condition, string From, string To, string type, string ttypr, string WithVal)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (WithVal == "WithAmt")
            {
                ...
                rptname.SetDataSource(dt);
                rptname.SetParameterValue("Comp", Session["CompanyName"].ToString());
                rptname.SetParameterValue("Title", "Vendor Stock Report Summary(With Value) From" + Convert.ToDateTime(From).ToString("dd/MMM/yyyy") + " To " + Convert.ToDateTime(To).ToString("dd/MMM/yyyy"));
            ...
            else
            {
                ...
                rptname.SetParameterValue("Comp", Session["CompanyName"].ToString());
                rptname.SetParameterValue("Title", "Vendor Stock Report Summary From " + Convert.ToDateTime(From).ToString("dd/MMM/yyyy") + " To " + Convert.ToDateTime(To).ToString("dd/MMM/yyyy"));
            }
            CrystalReportViewer1.ReportSource = rptname;
        }
        else
