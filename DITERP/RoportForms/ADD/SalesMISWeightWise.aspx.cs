...
public partial class RoportForms_ADD_SalesMISWeightWise : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string From, string To, string group, string way, string strCondition)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (group == "Datewise")
                {
                    rptname.Load(Server.MapPath("~/Reports/rptSalesWeightMISDatewise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptSalesWeightMISDatewise.rpt");
                    rptname.Refresh();
                ...
                if (group == "ItemWise")
                {
                    rptname.Load(Server.MapPath("~/Reports/rptSalesWeightMISItemwise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptSalesWeightMISItemwise.rpt");
                    rptname.Refresh();
                    rptname.SetDataSource(dt);
                    if (way == "Summary")
                    {
                ...
                if (group == "CustWise")
                {
                    rptname.Load(Server.MapPath("~/Reports/rptSalesWeightMISPartywise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptSalesWeightMISPartywise.rpt");
                    rptname.Refresh();
                    rptname.SetDataSource(dt);
                    if (way == "Summary")
                    {
                ...
                {
                    rptname.Load(Server.MapPath("~/Reports/rptSalesWeightMISPartyItemwise.rpt"));
                    rptname.FileName = Server.MapPath("~/Reports/rptSalesWeightMISPartyItemwise.rpt");
                    rptname.Refresh();
                    rptname.SetDataSource(dt);
                    if (way == "Summary")
                    {
