...
public partial class ToolRoom_ADD_Improvement : System.Web.UI.Page
{
    # region Variables
    ...
    protected void Upload(object sender, EventArgs e)
    {
        ...
        {
            sDirPath = Server.MapPath(@"~/UpLoadPath/Improvement/" + ViewState["mlCode"].ToString() + "/");
        }
        ...
        {
            FileUpload1.SaveAs(Server.MapPath("~/UpLoadPath/Improvement/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName"].ToString()));
        }
    }
    #endregion
    ...
    protected void Upload1(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString[0].Equals("INSERT"))
        {
            sDirPath = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp/");
        }
        else
        {
            sDirPath = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp/" + ViewState["mlCode"].ToString() + "/");
        }
        ...
        if (Request.QueryString[0].Equals("INSERT"))
        {
            FUDrawingTR.SaveAs(Server.MapPath("~/UpLoadPath/DrawingIMPUp/" + ViewState["fileName2"].ToString()));
        }
        else
        {
            FUDrawingTR.SaveAs(Server.MapPath("~/UpLoadPath/DrawingIMPUp/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName2"].ToString()));
        }
    }
    #endregion
    ...
    protected void lnkupload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                filePath = lnkupload.Text;
                directory = "../../UpLoadPath/Improvement/" + code + "/" + filePath;
        ...
        {
            CommonClasses.SendError("Improvement Entry", "lnkupload_Click", ex.Message);
        }
    }
    #endregion
    ...
    protected void lnkFUDrawingTR_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                directory = "../../UpLoadPath/DrawingIMPUp/" + filePath;
            }
            else
            {
                code = ViewState["mlCode"].ToString();
                filePath = lnkFUDrawingTR.Text;
                directory = "../../UpLoadPath/DrawingIMPUp/" + code + "/" + filePath;
            }
            ModalPopDocument.Show();
            IframeViewPDF.Attributes["src"] = directory;
        }
        ...
        {
            CommonClasses.SendError("Improvement Entry", "lnkFUDrawingTR_Click", ex.Message);
        }
    }
    #endregion
    ...
    public void loadPart()
    {
        ...
        Dt = CommonClasses.Execute("SELECT DISTINCT I_CODE, I_CODENO +' - '+ I_NAME AS I_CODENO FROM TOOL_MASTER,ITEM_MASTER where " + str + " TOOL_MASTER.ES_DELETE=0     AND T_CM_COMP_ID='" + Session["CompanyId"].ToString() + "' AND I_CODE=T_I_CODE   ");
        if (Dt.Rows.Count > 0)
        {
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "btnCancel_Click", ex.Message.ToString());
        }
    }
    ...
    private bool CheckValid()
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "CheckValid", Ex.Message);
        }
        return flag;
    }
    ...
    private void CancelRecord()
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "btnCancel_Click", ex.Message);
        }
    }
    #endregion
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (txtReason.Text.Trim() == "")
        {
            ...
            lblmsg.Text = "Please Enter NATURE OF IMPROVEMENT";
    ...
    private void ViewRec(string str)
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "ViewRec", ex.Message);
        }
    }
    #endregion ViewRec
    ...
    bool SaveRec()
    {
        ...
        try
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                dt1 = CommonClasses.Execute(" SELECT ISNULL(MAX(B_NO),0)+1 AS B_NO  FROM BREAKDOWN_ENTRY  WHERE  B_TYPE=1 AND    B_CM_CODE='" + Convert.ToInt32(Session["CompanyCode"].ToString()) + "' and   ES_DELETE='False'");
                if (dt1.Rows.Count > 0)
                {
                ...
                //if (dt.Rows.Count == 0)
                //{
                if (CommonClasses.Execute1("INSERT INTO BREAKDOWN_ENTRY (B_NO, B_SLIPNO, B_CM_CODE, B_CM_ID,B_T_TYPE, B_T_CODE,B_I_CODE, B_DATE, B_REASON, B_ACTION, B_CLOSURE, B_HOURS, B_FILE,B_TYPE,B_DRAWING_UPLOAD)VALUES('" + txtBNo.Text + "','" + txtSlipNo.Text.Trim().Replace("'", "\''") + "','" + Convert.ToInt32(Session["CompanyCode"]) + "','" + Convert.ToInt32(Session["CompanyId"]) + "' ,'" + ddlType.SelectedValue + "','" + ddlTool.SelectedValue + "','" + ddlPart.SelectedValue + "','" + Convert.ToDateTime(txtBreakDownDate.Text).ToString("dd/MMM/yyyy HH:mm") + "', '" + txtReason.Text.Trim().Replace("'", "\''") + "', '" + txtAction.Text.Trim().Replace("'", "\''") + "','" + Convert.ToDateTime(txtClosureDate.Text).ToString("dd/MMM/yyyy HH:mm") + "','" + txtHours.Text + "','" + lnkupload.Text + "',1,'" + lnkFUDrawingTR.Text + "')"))
                {
                    string Code = CommonClasses.GetMaxId("Select Max(B_CODE) from BREAKDOWN_ENTRY");
                    if (ViewState["fileName"].ToString().Trim() != "")
                    {
                        string sDirPath = Server.MapPath(@"~/UpLoadPath/Improvement/" + Code + "");
                        ...
                        string copyToPath = Server.MapPath(@"~/UpLoadPath/Improvement/" + Code + "/" + ViewState["fileName"].ToString().Trim());
                    ...
                    {
                        string sDirPath = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp/" + Code + "");

                        ObjSearchDir = new DirectoryInfo(sDirPath);

                        string sDirPath1 = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp");
                        DirectoryInfo dir = new DirectoryInfo(sDirPath1);
                        ...
                        //Get the full path of the file    
                        string fullFilePath1 = currentApplicationPath + "UpLoadPath\\DrawingIMPUp ";
                        string fullFilePath = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp/" + ViewState["fileName2"].ToString().Trim());
                        // Get the destination path
                        string copyToPath = Server.MapPath(@"~/UpLoadPath/DrawingIMPUp/" + Code + "/" + ViewState["fileName2"].ToString().Trim());
                        DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                        System.IO.File.Move(fullFilePath, copyToPath);
                    }

                    CommonClasses.WriteLog("Improvement Entry", "Save", "Improvement Entry", txtBNo.Text, Convert.ToInt32(txtBNo.Text), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                    result = true;
                    Response.Redirect("~/ToolRoom/View/ViewImprovement.aspx", false);
                }
                else
                ...
                //else
                //{
                //    ShowMessage("#Avisos", "Improvement Slip No. Already Exists", CommonClasses.MSG_Warning);
                //    ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                //}
            }
            #endregion INSERT
            ...
            else if (Request.QueryString[0].Equals("UpdateStatus"))
            {
                ...
                if (CommonClasses.Execute1("UPDATE BREAKDOWN_ENTRY SET B_ACTION='" + txtAction.Text.Trim().Replace("'", "\''") + "',B_CLOSURE='" + Convert.ToDateTime(txtClosureDate.Text).ToString("dd/MMM/yyyy HH:mm") + "',B_HOURS='" + txtHours.Text + "',B_FILE='" + lnkupload.Text + "' ,B_STATUS=1   WHERE B_CODE='" + ViewState["mlCode"].ToString() + "'"))
                {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "SaveRec", ex.Message);
        }
        return result;
    }
    #endregion
    ...
    string GetIP4Address()
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "GetIP4Address", ex.Message);
            return IP4Address;
        }
    }
    #endregion
    ...
    public bool ShowMessage(string DiveName, string Message, string MessageType)
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "ShowMessage", Ex.Message);
            return false;
        }
    }
    #endregion
    ...
    protected void ddlTool_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "ddlTool_SelectedIndexChanged", ex.Message);
    ...
    protected void ddlType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Improvement Entry", "ddlType_SelectedIndexChanged", ex.Message);
