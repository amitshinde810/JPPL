...
public partial class ToolRoom_ADD_Tools : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtcavity.Text.Trim() == "")
            {
                PanelMsg.Visible = true;
                lblmsg.Text = "Please Enter cavity ";
                ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                txtcavity.Focus();
                return;
            }
            ...
            {
                PanelMsg.Visible = true;
                lblmsg.Text = "Please Upload Tooling Photo";
                ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                FileUpload2.Focus();
                return;
            }
            ...
            {
                PanelMsg.Visible = true;
                lblmsg.Text = "Please Upload 3D Model";
                ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                FileUpload1.Focus();
                return;
            }
    ...
    private void ViewRec(string str)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["T_TYPE"].ToString().ToUpper() == "TRUE")
                {
                ...
                #region Validation_If_it_is_used_in_Transaction
                DataTable dtMP = CommonClasses.Execute("SELECT PM_CODE FROM MP_PREVENTIVE_MAINTENANCE_MASTER WHERE ES_DELETE=0 AND PM_TOOL_NO='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' AND PM_CM_COMP_ID='" + Session["CompanyId"].ToString() + "' union select B_CODE as PM_CODE from BREAKDOWN_ENTRY WHERE ES_DELETE=0 AND B_T_CODE='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and B_CM_ID='" + Session["CompanyId"].ToString() + "'");
                if (dtMP.Rows.Count == 0)
                { }
                else
                {
                    ddlCustomer.Enabled = false; txttoolno.Enabled = false;
                    ddlType.Enabled = false;
                    ddlpart.Enabled = false;
                }
                DataTable dtused = new DataTable();
                dtused = CommonClasses.Execute("SELECT * FROM  IRN_ENTRY INNER JOIN IRN_DETAIL ON IRN_CODE=IRND_IRN_CODE WHERE ES_DELETE=0 AND IRN_TRANS_TYPE=1 AND IRND_T_CODE='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "'");
                ...
                #endregion Validation_If_it_is_used_in_Transaction
    ...
    bool SaveRec()
    {
        ...
        try
        {
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                if (dt.Rows.Count == 0)
                { //trans = connection.BeginTransaction();
                    SqlCommand command = new SqlCommand("INSERT INTO TOOL_MASTER (T_NAME, T_TYPE, T_P_CODE, T_I_CODE, T_STDLIFE, T_PENDTOOLLIFE, T_PENDTOOLLIFEMONTH, T_OWNER,T_PMFREQ, T_REVNO, T_REV_DATE,T_CM_COMP_ID,T_STATUS,T_REF_NO,T_PHOTO_PATH,T_3D_PATH,T_TOOLNO,T_CRE_DATE)VALUES ('" + txttoolno.Text.Trim().Replace("'", "\''") + "','" + ddlType.SelectedValue.ToString() + "','" + ddlCustomer.SelectedValue.ToString() + "','" + ddlpart.SelectedValue.ToString() + "','" + txtStdToolLife.Text.Trim() + "','" + txtPendingShots.Text.Trim() + "','" + txtPendingMonth.Text.Trim() + "','" + ddlOwner.SelectedValue + "','" + txtPMfreq.Text.Trim() + "','" + txtRevNo.Text.Trim() + "','" + Convert.ToDateTime(txtRevDate.Text).ToString("dd/MMM/yyyy") + "','" + Convert.ToInt32(Session["CompanyId"]) + "','" + chkStatus.Checked + "','" + txtRefNo.Text.Trim().Replace("'", "\''") + "','" + lnkupload.Text + "','" + lnkTModel.Text + "','" + txtcavity.Text.Trim() + "',GETDATE())", connection, trans);
    ...
    private void LoadCust()
    {
        ...
        dt = CommonClasses.Execute("select distinct P_CODE,P_NAME from PARTY_MASTER  where " + str + " PARTY_MASTER.ES_DELETE=0 and P_TYPE=1 and P_ACTIVE_IND=1  and P_CM_COMP_ID=" + Session["CompanyID"] + "  order by P_NAME");
    ...
    private void LoadPartNo()
    {
        ...
        dt = CommonClasses.Execute("select distinct I_CODE,I_CODENO+'-'+I_NAME as I_NAME from ITEM_MASTER  where " + str + "   I_CM_COMP_ID=" + Session["CompanyID"] + "   and I_ACTIVE_IND=1 and I_CAT_CODE=-2147483648 order by I_NAME");
