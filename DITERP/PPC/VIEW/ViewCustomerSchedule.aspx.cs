...
public partial class Masters_VIEW_ViewCustomerSchedule : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='203'");
    ...
    private void LoadData()
    {
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE", "LoadUser", Ex.Message);
        }
    }
    #endregion
    ...
    protected void txtString_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE- View", "txtString_TextChanged", Ex.Message);
        }
    }
    #endregion
    ...
    private void LoadStatus(TextBox txtString)
    {
        try
        {
            ...
            DataTable dtfilter = new DataTable();
            if (txtString.Text != "")
                dtfilter = CommonClasses.Execute("SELECT DISTINCT C.CS_DATE AS ddd,CS_CODE, P_CODE,P_NAME,I_CODENO +' - '+ I_NAME AS PROD_NAME,I_CODE, right(convert(varchar, C.CS_DATE, 106), 8) as CS_DATE,I.I_CODE, ISNULL(C.CS_SCHEDULE_QTY, 0) AS CS_SCHEDULE_QTY, ISNULL(C.CS_WEEK1, 0) AS CS_WEEK1, ISNULL(C.CS_WEEK2, 0) AS CS_WEEK2, ISNULL(C.CS_WEEK3, 0) AS CS_WEEK3, ISNULL(C.CS_WEEK4, 0) AS CS_WEEK4 FROM CUSTOMER_SCHEDULE AS C INNER JOIN ITEM_MASTER AS I ON C.CS_I_CODE = I.I_CODE  inner join PARTY_MASTER P ON C.CS_P_CODE=P.P_CODE WHERE (C.ES_DELETE = 0) AND (I.ES_DELETE = 0) AND P.ES_DELETE =0 AND C.CS_COMP_ID='" + (string)Session["CompanyId"] + "'  AND C.CS_CM_CODE='" + (string)Session["CompanyCode"] + "'  and (lower(I_CODENO +' - '+ I_NAME) like lower('%" + str + "%') or (DATENAME(MM,CS_DATE) like lower('%" + str + "%')) or (DATENAME(YYYY,CS_DATE) like lower('%" + str + "%')) or (lower(P_NAME) like lower('%" + str + "%')))  ORDER BY C.CS_DATE DESC,P_NAME,I_CODENO +' - '+ I_NAME");
            else
                dtfilter = CommonClasses.Execute("SELECT DISTINCT C.CS_DATE AS ddd,CS_CODE, P_CODE,P_NAME,I_CODENO +' - '+ I_NAME AS PROD_NAME,I_CODE, right(convert(varchar, C.CS_DATE, 106), 8) as CS_DATE,I.I_CODE, ISNULL(C.CS_SCHEDULE_QTY, 0) AS CS_SCHEDULE_QTY, ISNULL(C.CS_WEEK1, 0) AS CS_WEEK1, ISNULL(C.CS_WEEK2, 0) AS CS_WEEK2, ISNULL(C.CS_WEEK3, 0) AS CS_WEEK3, ISNULL(C.CS_WEEK4, 0) AS CS_WEEK4 FROM CUSTOMER_SCHEDULE AS C INNER JOIN ITEM_MASTER AS I ON C.CS_I_CODE = I.I_CODE  inner join PARTY_MASTER P ON C.CS_P_CODE=P.P_CODE WHERE (C.ES_DELETE = 0) AND (I.ES_DELETE = 0) AND P.ES_DELETE =0 AND C.CS_COMP_ID='" + (string)Session["CompanyId"] + "'  AND C.CS_CM_CODE='" + (string)Session["CompanyCode"] + "'   ORDER BY C.CS_DATE DESC,P_NAME,I_CODENO +' - '+ I_NAME");
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE - View", "LoadStatus", Ex.Message);
        }
    }
    #endregion
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string type = "INSERT";
                Response.Redirect("~/PPC/ADD/CustomerSchedule.aspx?c_name=" + type, false);
            }
            else
            {
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE", "btnInsert_Click", exc.Message);
        }
    }
    #endregion
    ...
    protected void dgCustomerSchedule_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName.Equals("View"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For View"))
                {
                    ...
                    Response.Redirect("~/PPC/ADD/CustomerSchedule.aspx?c_name=" + type + "&u_code=" + um_code, false);
                }
                else
                {
                    PanelMsg.Visible = true;
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE", "dgCustomerSchedule_RowCommand", exc.Message);
        }
    }
    ...
    bool ModifyLog(string PrimaryKey)
    {
        try
        {
            ...
            DataTable dt = CommonClasses.Execute("select MODIFY from CUSTOMER_SCHEDULE where CS_CODE=" + PrimaryKey + "  ");
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE", "dgCustomerSchedule_RowEditing", exc.Message);
        }
        return false;
    }
    #endregion
    ...
    protected void dgCustomerSchedule_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(4, 1)), this, "For Delete"))
            {
                ...
                {
                    string um_code = ((Label)(dgCustomerSchedule.Rows[e.RowIndex].FindControl("lblCS_CODE"))).Text;

                    string CS_DATE = ((Label)(dgCustomerSchedule.Rows[e.RowIndex].FindControl("lblCS_DATE"))).Text;

                    string I_CODE = ((Label)(dgCustomerSchedule.Rows[e.RowIndex].FindControl("lblI_CODE"))).Text;

                    DataTable DtDetails = CommonClasses.Execute("SELECT * FROM VENDOR_SCHEDULE where ES_DELETE=0 AND VS_I_CODE='" + I_CODE + "' AND VS_DATE='" + Convert.ToDateTime(CS_DATE).ToString("dd/MMM/yyyy") + "'");

                    if (DtDetails.Rows.Count > 0)
                    {
                        PanelMsg.Visible = true;
                        lblmsg.Text = "You Can Not Delete ,This is Used In Vendor Schedule.";
                        ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                        return;
                    }
                    DataTable DtDetail = CommonClasses.Execute("SELECT * FROM DAILY_SALE_ENTRY_MASTER,DAILY_SALE_ENTRY_DETAIL where DSEM_CODE=DSED_DSEM_CODE AND ES_DELETE=0 AND DATEPART(MM,DSEM_MONTH)='" + Convert.ToDateTime(CS_DATE).Month + "' AND DATEPART(YYYY,DSEM_MONTH)='" + Convert.ToDateTime(CS_DATE).Year + "' AND DSED_I_CODE ='" + I_CODE + "'  ");

                    if (DtDetail.Rows.Count > 0)
                    {
                        PanelMsg.Visible = true;
                        lblmsg.Text = "You Can Not Delete ,This is Used In Daily Sales Plan. ";
                        ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                        return;
                    } 

                    bool flag = CommonClasses.Execute1("UPDATE CUSTOMER_SCHEDULE SET ES_DELETE = 1 WHERE CS_CODE='" + Convert.ToInt32(um_code) + "'");
                    if (flag == true)
                    {
                        CommonClasses.WriteLog("CUSTOMER SCHEDULE", "Delete", "CUSTOMER SCHEDULE", um_code, Convert.ToInt32(um_code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                    }
                    LoadData();
                    PanelMsg.Visible = true;
    ...
    protected void btnClose_Click(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("CUSTOMER SCHEDULE", "btnClose_Click", ex.Message);
        }
    }
