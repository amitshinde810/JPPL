...
public partial class PPC_VIEW_ViewCoreInventoryRegister : System.Web.UI.Page
{
    static string right = "";
    ...
    private void LoadCombos()
    {
        ...
        dtPartName = CommonClasses.Execute("select DISTINCT I.I_CODE,I.I_CODENO + ' - ' + I.I_NAME AS ICODE_INAME from CORE_INVENTORY C inner join ITEM_MASTER I on c.CORE_I_CODE=I.I_CODE inner join PRODUCT_MASTER PROD on I.I_CODE=PROD.PROD_I_CODE INNER JOIN GROUP_MASTER GP ON PROD.PROD_GP_CODE=GP.GP_CODE where  C.ES_DELETE=0 AND I.ES_DELETE=0 AND PROD.ES_DELETE=0 AND GP.ES_DELETE=0 and C.CORE_MONTH='" + Convert.ToDateTime(txtMonth.Text).ToString("dd/MMM/yyyy") + "' AND C.CORE_COMP_ID='" + (string)Session["CompanyId"] + "' ORDER BY I.I_CODE,I.I_CODENO + ' - ' + I.I_NAME");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 5)), this, "For Menu"))
            {
                ...
                Response.Redirect("~/PPC/ReportForms/ADD/CoreInventory.aspx?Title=" + Title + "&Date=" + txtMonth.Text + "&StrCond=" + StrCond + "", false);
            }
            else
            {
