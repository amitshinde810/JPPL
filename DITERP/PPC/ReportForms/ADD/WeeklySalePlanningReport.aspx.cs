...
public partial class PPC_ReportForms_ADD_WeeklySalePlanningReport : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string Date, string StrCond)
    {
        try
        {
            ...
            string Query = "";
            /*Check ActualStockQty from CUSTOMER_WEEKLY_PLAN*/
            double NoofDaysAvailable = 25; /* Bind Dynamically*/
            //Query = "SELECT DISTINCT G.GP_NAME,I.I_CODE,I_CODENO ,I_NAME ,isnull(CWP_SALE_QTY,0) as SALE_PLAN_QTY,ISNULL((select SUM(ISNULL(CPOD_ORD_QTY,0)) from CUSTPO_MASTER CPOM INNER JOIN CUSTPO_DETAIL CPOD ON CPOM.CPOM_CODE=CPOD.CPOD_CPOM_CODE WHERE CPOD.CPOD_I_CODE=I.I_CODE AND CPOM.ES_DELETE=0 AND datepart(MONTH,CPOM.CPOM_PO_DATE)='" + dtMonth.ToString("MM") + "' and datepart(year,CPOM.CPOM_PO_DATE)='" + dtMonth.ToString("yyyy") + "'),0) AS SALE_PLAN_QTY_From_ERP,ISNULL(SIM_CUST_STOCK,0) AS Tot_Stnd_Stock ,ISNULL((CWP_CUSTOMER_STOCK),0) AS ActualStockQty ,case when CWP_WEEK='1' then 'Week :1' when CWP_WEEK='2' then 'Week :2' when CWP_WEEK='3' then 'Week :3' when CWP_WEEK='4' then 'Week :4' else '0' end As CWP_WEEK ,right(convert(varchar,CWP.CWP_DATE,106),8) as CWP_DATE FROM GROUP_MASTER G INNER JOIN STANDARD_INVENTARY_MASTER S ON G.GP_CODE =S.SIM_GP_CODE INNER JOIN ITEM_MASTER I ON S.SIM_I_CODE=I.I_CODE INNER JOIN CUSTOMER_SCHEDULE C ON SIM_I_CODE=C.CS_I_CODE inner join PRODUCT_MASTER PROD on S.SIM_I_CODE=PROD.PROD_I_CODE inner join CUSTOMER_WEEKLY_PLAN CWP ON I.I_CODE=CWP.CWP_I_CODE WHERE " + StrCond + " S.ES_DELETE=0 AND CWP.ES_DELETE=0 AND I.ES_DELETE=0 AND G.ES_DELETE=0 AND PROD.ES_DELETE=0 AND S.SIM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' and CWP.CWP_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "'";
            Query = "SELECT DISTINCT G.GP_NAME,I.I_CODE,I_CODENO ,isnull((select sum(isnull(I_INV_RATE,0)) from ITEM_MASTER WHERE ITEM_MASTER.ES_DELETE=0 AND ITEM_MASTER.I_CODE=I.I_CODE),0) as I_INV_RATE,I_NAME ,isnull((select sum(isnull(CWP_SALE_QTY,0)) from CUSTOMER_WEEKLY_PLAN where CWP_I_CODE=I.I_CODE and CUSTOMER_WEEKLY_PLAN.ES_DELETE=0 and CWP_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "' AND CWP_WEEK=CWP.CWP_WEEK),0) as SALE_PLAN_QTY ,ISNULL(SIM_CUST_STOCK,0) as Tot_Stnd_Stock,isnull((select isnull(sum(CWP_CUSTOMER_STOCK),0) from CUSTOMER_WEEKLY_PLAN where CWP_I_CODE=I.I_CODE and CUSTOMER_WEEKLY_PLAN.ES_DELETE=0 and CWP_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "' AND CWP_WEEK=CWP.CWP_WEEK),0) AS ActualStockQty ,case when CWP_WEEK='1' then 'Week :1' when CWP_WEEK='2' then 'Week :2' when CWP_WEEK='3' then 'Week :3' when CWP_WEEK='4' then 'Week :4' else '0' end As CWP_WEEK ,right(convert(varchar,CWP.CWP_DATE,106),8) as CWP_DATE FROM GROUP_MASTER G INNER JOIN STANDARD_INVENTARY_MASTER S ON G.GP_CODE =S.SIM_GP_CODE INNER JOIN ITEM_MASTER I ON S.SIM_I_CODE=I.I_CODE INNER JOIN CUSTOMER_SCHEDULE C ON SIM_I_CODE=C.CS_I_CODE inner join PRODUCT_MASTER PROD on S.SIM_I_CODE=PROD.PROD_I_CODE inner join CUSTOMER_WEEKLY_PLAN CWP ON I.I_CODE=CWP.CWP_I_CODE WHERE " + StrCond + " PROD.ES_DELETE=0 and S.ES_DELETE=0 AND CWP.ES_DELETE=0 AND I.ES_DELETE=0 AND G.ES_DELETE=0 AND PROD.ES_DELETE=0 AND S.SIM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' and CWP.CWP_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "' group by G.GP_NAME,I.I_CODE,I_CODENO ,I_NAME,CWP_WEEK,SIM_CUST_STOCK,right(convert(varchar,CWP.CWP_DATE,106),8) order by G.GP_NAME,I_CODENO ,I_NAME"; //PROD.PROD_MACHINE_LOC in('INHOUSE','BOTH') and
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("~/PPC/ReportForms/VIEW/ViewWeeklySalePlanningReport.aspx", false);
        }
        catch (Exception Ex)
        {
