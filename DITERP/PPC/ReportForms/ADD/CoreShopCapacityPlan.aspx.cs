...
public partial class RoportForms_ADD_CoreShopCapacityPlan : System.Web.UI.Page
{
    ...
    private void GenerateReport(string Title, string Date, string StrCond)
    {
        try
        {
            ...
            Query = "SELECT * into #Temp from(SELECT DISTINCT I.I_CODE,I_CODENO,I_NAME ,ISNULL(PROD_CORE_WT,0) as PROD_CORE_WT,ROUND((round(isnull((isnull((isnull((select SUM(ISNULL(CS_SCHEDULE_QTY,0)) FROM CUSTOMER_SCHEDULE where CUSTOMER_SCHEDULE.CS_I_CODE=I.I_CODE and CUSTOMER_SCHEDULE.ES_DELETE=0 and CS_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "'),0)  + ISNULL((ISNULL(SIM_RFI_STORE,0)+ ISNULL(SIM_FINISH_GOODS,0)+ ISNULL(SIM_MACHINE_SHOP,0)+ISNULL(SIM_VENDOR,0) +ISNULL(SIM_RFM_STORE,0) +ISNULL(SIM_FOUNDRY,0) +ISNULL(SIM_MAIN_STORE,0) ),0)- ISNULL((select SUM(ISNULL(STL_DOC_QTY,0)) AS STL_DOC_QTY from STOCK_LEDGER WHERE STL_I_CODE= I.I_CODE and STL_DOC_DATE<'" + dtMonth.ToString("dd/MMM/yyyy") + "'),0)),0)) * isnull((SELECT ISNULL((CAST(RM_CASTING_REJECTION AS FLOAT)/100)+1,0) FROM REJECTION_MASTER WHERE ES_DELETE=0 AND RM_COMP_ID='" + Session["CompanyId"] + "' AND RM_DATE=(SELECT MAX(RM_DATE) FROM REJECTION_MASTER WHERE ES_DELETE=0 AND RM_COMP_ID='" + Session["CompanyId"] + "')),0) +0 -ISNULL((select SUM(ISNULL(STL_DOC_QTY,0)) AS STL_DOC_QTY from STOCK_LEDGER WHERE STL_STORE_TYPE = '-2147483646' AND STL_I_CODE= I.I_CODE and STL_DOC_DATE<'" + dtMonth.ToString("dd/MMM/yyyy") + "'),0) ,0),0) * isnull((SELECT ISNULL((CAST(RM_CORE_BREAKAGE AS FLOAT)/100)+1,0) FROM REJECTION_MASTER WHERE ES_DELETE=0 AND RM_COMP_ID='" + Session["CompanyId"] + "' AND RM_DATE=(SELECT MAX(RM_DATE) FROM REJECTION_MASTER WHERE ES_DELETE=0 AND RM_COMP_ID='" + Session["CompanyId"] + "')),0) ),0) AS CorestobeMade ,isnull((select ISNULL(MBD_NO_OF_UNITS,0) AS MBD_NO_OF_UNITS from MACHINE_BOOKING_MASTER MBM inner join MACHINE_BOOKING_DETAIL MBD on MBM.MBM_CODE=MBD.MBD_MBM_CODE WHERE MBM.MBM_I_CODE=I.I_CODE AND MBM.ES_DELETE=0 and MBD_M_CODE=-2147483648 AND MBM.MBM_PMN_CODE=-2147483647),0) as [800x600],isnull((select ISNULL(MBD_NO_OF_UNITS,0) AS MBD_NO_OF_UNITS from MACHINE_BOOKING_MASTER MBM inner join MACHINE_BOOKING_DETAIL MBD on MBM.MBM_CODE=MBD.MBD_MBM_CODE WHERE MBM.MBM_I_CODE=I.I_CODE AND MBM.ES_DELETE=0 and MBD_M_CODE=-2147483647 AND MBM.MBM_PMN_CODE=-2147483647),0) as [600x450],isnull((select ISNULL(MBD_NO_OF_UNITS,0) AS MBD_NO_OF_UNITS from MACHINE_BOOKING_MASTER MBM inner join MACHINE_BOOKING_DETAIL MBD on MBM.MBM_CODE=MBD.MBD_MBM_CODE WHERE MBM.MBM_I_CODE=I.I_CODE AND MBM.ES_DELETE=0 and MBD_M_CODE=-2147483646 AND MBM.MBM_PMN_CODE=-2147483647),0) as [600x300],isnull((select ISNULL(MBD_NO_OF_UNITS,0) AS MBD_NO_OF_UNITS from MACHINE_BOOKING_MASTER MBM inner join MACHINE_BOOKING_DETAIL MBD on MBM.MBM_CODE=MBD.MBD_MBM_CODE WHERE MBM.MBM_I_CODE=I.I_CODE AND MBM.ES_DELETE=0 and MBD_M_CODE=-2147483645 AND MBM.MBM_PMN_CODE=-2147483647),0) as [500x350],ISNULL((SELECT isnull(PMM_NO_MACHINES,0) as PMM_NO_MACHINES FROM PROCESS_MACHINE_MASTER PMM where PMM_P_ID=-2147483647 and PMM.ES_DELETE=0 and PMM_M_ID=-2147483648),0) AS [800x600_Available],ISNULL((SELECT isnull(PMM_NO_MACHINES,0) as PMM_NO_MACHINES FROM PROCESS_MACHINE_MASTER PMM where PMM_P_ID=-2147483647 and PMM.ES_DELETE=0 and PMM_M_ID=-2147483647),0) AS [600x450_Available],ISNULL((SELECT isnull(PMM_NO_MACHINES,0) as PMM_NO_MACHINES FROM PROCESS_MACHINE_MASTER PMM where PMM_P_ID=-2147483647 and PMM.ES_DELETE=0 and PMM_M_ID=-2147483646),0) AS [600x300_Available],ISNULL((SELECT isnull(PMM_NO_MACHINES,0) as PMM_NO_MACHINES FROM PROCESS_MACHINE_MASTER PMM where PMM_P_ID=-2147483647 and PMM.ES_DELETE=0 and PMM_M_ID=-2147483645),0) AS [500x350_Available] FROM GROUP_MASTER G INNER JOIN STANDARD_INVENTARY_MASTER S ON G.GP_CODE =S.SIM_GP_CODE INNER JOIN ITEM_MASTER I ON S.SIM_I_CODE=I.I_CODE INNER JOIN CUSTOMER_SCHEDULE C ON SIM_I_CODE=C.CS_I_CODE inner join PRODUCT_MASTER PROD on S.SIM_I_CODE=PROD.PROD_I_CODE  WHERE  S.ES_DELETE=0 AND I.ES_DELETE=0 AND G.ES_DELETE=0 AND PROD.ES_DELETE=0 AND S.SIM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' and CS_DATE='" + dtMonth.ToString("dd/MMM/yyyy") + "' ) as Temp select I_CODE,I_CODENO,I_NAME,PROD_CORE_WT,CorestobeMade,[800x600],[600x450],[600x300],[500x350],case when [800x600]=0 then 0 else round(CorestobeMade/[800x600],2) end as [800x600_Booking] ,case when [600x450]=0 then 0 else round(CorestobeMade/[600x450],2) end as [600x450_Booking] ,case when [600x300]=0 then 0 else round(CorestobeMade/[600x300],2) end as [600x300_Booking] ,case when [500x350]=0 then 0 else round(CorestobeMade/[500x350],2) end as [500x350_Booking],[800x600_Available],[600x450_Available],[600x300_Available],[500x350_Available] into #Temp1 from #Temp ORDER BY #Temp.I_CODENO, I_NAME drop table #Temp select I_CODE,I_CODENO,I_NAME,PROD_CORE_WT,case when CorestobeMade<0 then 0 else CorestobeMade end as CorestobeMade, [800x600],[600x450],[600x300],[500x350],case when [800x600_Booking]<0 then 0 else [800x600_Booking] end as [800x600_Booking],case when [600x450_Booking]<0 then 0 else [600x450_Booking] end as [600x450_Booking] ,case when [600x300_Booking]<0 then 0 else [600x300_Booking] end as [600x300_Booking],case when [500x350_Booking]<0 then 0 else [500x350_Booking] end as [500x350_Booking],[800x600_Available],[600x450_Available],[600x300_Available],[500x350_Available] from #Temp1 drop table #Temp1";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                rptname.Load(Server.MapPath("~/PPC/Reports/CoreShopCapacityPlan.rpt"));
                rptname.FileName = Server.MapPath("~/PPC/Reports/CoreShopCapacityPlan.rpt");
                rptname.Refresh();
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("~/PPC/ReportForms/VIEW/ViewCoreShopCapacityPlan.aspx", false);
        }
        catch (Exception Ex)
        {
