<%@ Page Language="C#" MasterPageFile="~/main.master" AutoEventWireup="true" CodeFile="ViewCustomerPO.aspx.cs"
    Inherits="Transaction_VIEW_ViewCustomerPO" Title="Customer PO" %>
                                ...
                                <ContentTemplate>
                                    <asp:GridView ID="dgDetailPO" runat="server" AutoGenerateColumns="False" Width="100%"
                                        ...
                                        <Columns>
                                                            ...
                                                            <ul class="dropdown-menu" role="menu">
                                                                ...
                                                                <li>
                                                                    <asp:LinkButton ID="lnkAmend" runat="server" CausesValidation="False" CommandName="AMEND"
                                                                        Visible="false" Text="Amend" CommandArgument='<%# Bind("CPOM_CODE") %>'><i class="fa fa-link"></i> Amend</asp:LinkButton>
                                                                </li>
                                                                <li>
                                            ...
                                            <asp:TemplateField HeaderText="Status" SortExpression="CPOD_STATUS" HeaderStyle-HorizontalAlign="Left">
                                                <ItemTemplate>
                                                    <asp:Label ID="lblCPOD_STATUS" CssClass=" Control-label pull-left" runat="server"
                                                        Text='<%# Eval("CPOD_STATUS") %>'></asp:Label>
                                                </ItemTemplate>
                                                <HeaderStyle HorizontalAlign="Left" CssClass="text-left" />
                                            </asp:TemplateField>
                                            
                                             <asp:TemplateField HeaderText="Approve" SortExpression="CPOM_APPROVE" HeaderStyle-HorizontalAlign="Left">
                                                <ItemTemplate>
                                                    <asp:LinkButton ID="lnkCPOM_APPROVE" CssClass='<%# (Convert.ToString(Eval("CPOM_APPROVE"))=="Approve")? "btn green btn-xs":"btn blue btn-xs" %>'
                                                        BorderStyle="None" runat="server" CausesValidation="False" CommandName="Approve"
                                                        Text='<%#  Eval("CPOM_APPROVE") %>' CommandArgument='<%# ((GridViewRow)Container).RowIndex%>'></asp:LinkButton>
                                                </ItemTemplate>
                                                <HeaderStyle HorizontalAlign="Left" CssClass="text-left" />
                                            </asp:TemplateField>
                                            <asp:TemplateField HeaderText="Authorize" SortExpression="CPOM_AUTH_FLG" HeaderStyle-HorizontalAlign="Left">
                                                <ItemTemplate>
                                                    <asp:LinkButton ID="lnkCPOM_AUTH_FLG" CssClass='<%# (Convert.ToString(Eval("CPOM_AUTH_FLG"))=="Authorize")? "btn green btn-xs":"btn blue btn-xs" %>'
                                                        BorderStyle="None" runat="server" CausesValidation="False" CommandName="Authorize"
                                                        Text='<%#  Eval("CPOM_AUTH_FLG") %>' CommandArgument='<%# ((GridViewRow)Container).RowIndex%>'></asp:LinkButton>
                                                </ItemTemplate>
                                                <HeaderStyle HorizontalAlign="Left" CssClass="text-left" />
                                            </asp:TemplateField>
