...
public partial class Transactions_VIEW_ViewForProcessInward : System.Web.UI.Page
{
    ...
    protected void btnAddNew_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string type = "INSERT";
                Response.Redirect("~/Transactions/ADD/ForProcessInward.aspx?c_name=" + type, false);
            }
            else
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (flag == true)
                        {
                            ...
                            {
                                CommonClasses.Execute("update CUSTPO_DETAIL set CPOD_DISPACH=CPOD_DISPACH-" + dtq.Rows[i]["IWD_REV_QTY"] + " where CPOD_I_CODE='" + dtq.Rows[i]["IWD_I_CODE"] + "' and CPOD_CPOM_CODE='" + dtq.Rows[i]["IWD_CPOM_CODE"] + "'");
                                ...
                                CommonClasses.Execute1("DELETE FROM CHALLAN_STOCK_LEDGER WHERE CL_DOC_ID='" + cpom_code + "' and CL_DOC_TYPE='IWIFP' AND CL_I_CODE ='" + dtq.Rows[i]["IWD_I_CODE"] + "'");
                            }

                            flag = CommonClasses.Execute1("DELETE FROM STOCK_LEDGER WHERE STL_DOC_NO='" + cpom_code + "' and STL_DOC_TYPE='IWIFP'");
    ...
    protected void dgDetailPO_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("View"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For View"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        Response.Redirect("~/Transactions/ADD/ForProcessInward.aspx?c_name=" + type + "&cpom_code=" + cpom_code, false);
                    }
                    else
                    {
                        PanelMsg.Visible = true;
            ...
            {
                string cpom_code = e.CommandArgument.ToString();
                Response.Redirect("~/RoportForms/ADD/RptInspectionGIN_IWIM.aspx?Title=" + Title + "&cpom_code=" + cpom_code, false);
            }
        }
        catch (Exception Ex)
        {
