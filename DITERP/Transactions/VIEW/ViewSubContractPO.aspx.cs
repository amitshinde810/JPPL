...
public partial class Transactions_VIEW_ViewSubContractPO : System.Web.UI.Page
{
    ...
    protected void btnAddNew_Click(object sender, EventArgs e)
    {
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(3, 1)), this, "For Add"))
            {
                ...
                string type = "INSERT";
                Response.Redirect("~/Transactions/ADD/SubContractPO.aspx?c_name=" + type, false);
            }
            else
            {
    ...
    protected void dgDetailSupplierPO_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            string type = "MODIFY";
            Response.Redirect("~/Transactions/ADD/SubContractPO.aspx?c_name=" + type + "&cpom_code=" + cpom_code, false);
        }
        catch (Exception Ex)
        {
        ...
        try
        {
            if (CommonClasses.ValidRights(int.Parse(right.Substring(4, 1)), this, "For Delete"))
            {
                if (!ModifyLog(((Label)(dgDetailSupplierPO.Rows[e.RowIndex].FindControl("lblCPOM_CODE"))).Text))
                {
                    ...
                    else if (CommonClasses.CheckUsedInTran("INVOICE_MASTER,INVOICE_DETAIL", "IND_CPOM_CODE", " AND INVOICE_MASTER.ES_DELETE=0 AND  INM_CODE=IND_INM_CODE AND INM_TYPE IN ('OutSUBINM' )", cpom_code))
                    {
                        PanelMsg.Visible = true;
                        lblmsg.Text = "Record not deleted, it is used in Dispatch";
                        ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                        return;
                    }
                    else
    ...
    protected void dgDetailSupplierPO_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            else if (e.CommandName.Equals("Modify"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(2, 1)), this, "For Modify"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        {
                            {
                                DataTable dtPOCancel = CommonClasses.Execute("SELECT ISNULL(SPOM_CANCEL_PO,0) AS SPOM_CANCEL_PO FROM SUPP_PO_MASTER WHERE SPOM_CODE='" + cpom_code + "'");
                                if (CommonClasses.CheckUsedInTran("INVOICE_MASTER,INVOICE_DETAIL", "IND_CPOM_CODE", "AND INM_CODE=IND_INM_CODE AND INM_TYPE='OutSUBINM' AND INVOICE_MASTER.ES_DELETE=0", cpom_code))
                                {
                                    PanelMsg.Visible = true;
                                    lblmsg.Text = "You can't Modify this record, it is used in Sub Contractor Distpach";
                                    ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                                    return;
                                }
                                else if (dtPOCancel.Rows[0]["SPOM_CANCEL_PO"].ToString() == "True")
                                {
                                    PanelMsg.Visible = true;
                ...
                if (dtAutoFlag.Rows[0]["SPOM_AUTHR_FLAG"].ToString() == "True")
                {
                    if (CommonClasses.ValidRights(int.Parse(right.Substring(5, 1)), this, "For Modify"))
                    {
                        if (!ModifyLog(e.CommandArgument.ToString()))
                        {
                            ...
                            Response.Redirect("~/RoportForms/ADD/SubContractorPOPrint.aspx?cpom_code=" + cpom_code + "&AuthoType=" + AuthoFlag + "&PoType=" + PoType, false);
                        }
                    }
                    else
                    {
            ...
            else if (e.CommandName.Equals("Authorize"))
            {
                ...
                string qe_no = ((Label)dgDetailSupplierPO.Rows[index].FindControl("lblCPOM_PONO")).Text;
                ...
                if (CommonClasses.ValidRights(int.Parse(right1.Substring(2, 1)), this, "For Authorize"))
                {
                    ...
                    CommonClasses.WriteLog("Sub Contractor Purchase Order", "Authorize", "Sub Contractor Purchase Order", Convert.ToString(qe_no), Convert.ToInt32(qe_code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                    LoadStatus(txtString);
                }
                else
                {
                ...
                if (CommonClasses.ValidRights(int.Parse(right.Substring(2, 1)), this, "For Post"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        if (CommonClasses.ValidRights(int.Parse(right1.Substring(2, 1)), this, "For Post"))
                        {
                            ...
                            {
                                CommonClasses.WriteLog("Sub Contractor Purchase Order", "POST", "Sub Contractor Purchase Order", Convert.ToString(cpom_code), Convert.ToInt32(cpom_code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                ...
                if (CommonClasses.ValidRights(int.Parse(right.Substring(2, 1)), this, "For Amend"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                Response.Redirect("~/Transactions/ADD/SubContractPO.aspx?c_name=" + type1 + "&cpom_code=" + cpom_code, false);
                            }
                            return;
                        }
                        else
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            if ((dt.Rows[0]["SPOM_POST"]).ToString() == "True")
                            {
                                ...
                                if (INW_QTY > 0)
                                {
                                    ...
                                    if (Res == true)
                                    {
                                        CommonClasses.WriteLog("Sub Contractor Purchase Order", "ShortClose", "Sub Contractor Purchase Order", Convert.ToString(cpom_code), Convert.ToInt32(cpom_code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                                        PanelMsg.Visible = true;
                ...
                if (CommonClasses.ValidRights(int.Parse(right.Substring(2, 1)), this, "For CancelPO"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                CommonClasses.WriteLog("Sub Contractor Purchase Order", "CancelPO", "Sub Contractor Purchase Order", Convert.ToString(cpom_code), Convert.ToInt32(cpom_code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                                LoadSupplierPO();
                                PanelMsg.Visible = true;
    ...
    protected void ddlFilter_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlFilter.SelectedValue == "4")//Cancel
        {
            dataView.RowFilter = "SPOM_CANCEL_PO=1";
        }
        else if (ddlFilter.SelectedValue == "3")//ShortClose
        {
            dataView.RowFilter = "SPOM_IS_SHORT_CLOSE=1";
        }
        else if (ddlFilter.SelectedValue == "2")//Active
        {
            dataView.RowFilter = "SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0";
        }
        else if (ddlFilter.SelectedValue == "0")//All
        {
            LoadSupplierPO();
        }
