    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='262'");
        ...
        if (CommonClasses.ValidRights(int.Parse(right.Substring(4, 1)), this, "For Delete"))
        {
            if (!ModifyLog(((Label)(dgDetailDC.Rows[e.RowIndex].FindControl("lblDCM_CODE"))).Text))
            {
                try
                {
                    ...
                    if (flag == true)
                    {
                        DataTable dtq = CommonClasses.Execute("SELECT DND_REC_QTY  ,DND_SCRAP_QTY,DND_REC_QTY+DND_SCRAP_QTY AS TOTAL_QTY ,DND_I_CODE,DNM_PARTY_DC_NO,DND_STORE_CODE  FROM DC_RETURN_DETAIL ,DC_RETURN_MASTER where DNM_CODE=DND_DNM_CODE  AND DND_DNM_CODE=" + Convert.ToInt32(cpom_code) + " ");
