...
public partial class Transactions_ADD_LabourChargeInvoice : System.Web.UI.Page
{
    ...
    private void LoadICode()
    {
        try
        {
            dt = CommonClasses.Execute("select distinct I_CODE,I_CODENO from ITEM_MASTER,CUSTPO_DETAIL,CUSTPO_MASTER where CUSTPO_MASTER.ES_DELETE=0 and  ((CPOD_ORD_QTY-CPOD_DISPACH)>0 or CPOM_CODE=CPOD_CPOM_CODE) and CPOD_I_CODE=I_CODE and  CPOM_CODE=CPOD_CPOM_CODE and CPOM_P_CODE=" + ddlCustomer.SelectedValue + " and CPOM_IS_VERBAL=0 AND CPOM_TYPE=-2147483648 order by I_CODENO ASC");
    ...
    private void LoadIName()
    {
        try
        {
            dt = CommonClasses.Execute("select distinct I_CODE,I_NAME from ITEM_MASTER,CUSTPO_DETAIL,CUSTPO_MASTER where CUSTPO_MASTER.ES_DELETE=0 and  ((CPOD_ORD_QTY-CPOD_DISPACH)>0 or CPOM_CODE=CPOD_CPOM_CODE) and  CPOD_I_CODE=I_CODE   and CPOM_CODE=CPOD_CPOM_CODE  and CPOM_P_CODE=" + ddlCustomer.SelectedValue + " and CPOM_IS_VERBAL=0 AND CPOM_TYPE=-2147483648 order by I_NAME ASC");
    ...
    protected void ddlItemCode_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlItemCode.SelectedIndex != 0)
            {
                ...
                DataTable dtStkQty = CommonClasses.Execute("select isnull(sum(STL_DOC_QTY),0) as STL_DOC_QTY from STOCK_LEDGER where STL_STORE_TYPE=-2147483642 and STL_I_CODE='" + ddlItemCode.SelectedValue + "'");

                DataTable dt1 = CommonClasses.Execute("Select ITEM_UNIT_MASTER.I_UOM_CODE,I_UOM_NAME,I_UWEIGHT, ISNULL(I_DISPATCH_BAL,0) AS I_CURRENT_BAL,I_SCAT_CODE from ITEM_UNIT_MASTER,ITEM_MASTER where ITEM_MASTER.I_UOM_CODE=ITEM_UNIT_MASTER.I_UOM_CODE and ITEM_MASTER.ES_DELETE=0 and I_CODE='" + ddlItemCode.SelectedValue + "'");
                DataTable dtPO = new DataTable();
                if (Request.QueryString[0].Equals("MODIFY"))
                {
                ...
                {
                    dtPO = CommonClasses.Execute("SELECT DISTINCT(CPOM_CODE),CPOM_PONO FROM CUSTPO_MASTER,CUSTPO_DETAIL WHERE CPOM_CODE=CPOD_CPOM_CODE and CPOD_I_CODE='" + ddlItemCode.SelectedValue + "' AND CPOM_P_CODE='" + ddlCustomer.SelectedValue + "' AND CUSTPO_MASTER.ES_DELETE=0 and ES_DELETE=0    and CPOM_IS_VERBAL=0  AND CPOM_AUTH_FLG=1");
                }
                ddlPONo.DataSource = dtPO;
                ddlPONo.DataTextField = "CPOM_PONO";
                ddlPONo.DataValueField = "CPOM_CODE";
    ...
    protected void ddlPONo_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlItemCode.SelectedIndex != 0)
            {
                ...
                //DataTable dtQty = CommonClasses.Execute("SELECT (CPOD_DISPACH) as Qty FROM CUSTPO_DETAIL where CPOD_I_CODE=" + ddlItemCode.SelectedValue + " and CPOD_CPOM_CODE=" + ddlPONo.SelectedValue + "  ");
                //DataTable dtInvQty = CommonClasses.Execute("SELECT ISNULL(SUM(IND_INQTY),0) AS IND_INQTY FROM INVOICE_MASTER,INVOICE_DETAIL WHERE INM_CODE=IND_INM_CODE AND  INVOICE_MASTER.ES_DELETE=0 AND INM_TYPE='OutJWINM' AND IND_I_CODE=" + ddlItemCode.SelectedValue + " and IND_CPOM_CODE=" + ddlPONo.SelectedValue + " ");
                double inmQTy = 0;
                ...
                else
                {
                    ...
                    dtQty = CommonClasses.Execute("SELECT ISNULL(SUM(CL_CQTY),0) as Qty FROM CUSTPO_DETAIL,CHALLAN_STOCK_LEDGER,CUSTPO_MASTER,INWARD_MASTER,INWARD_DETAIL where CPOD_CPOM_CODE='" + ddlPONo.SelectedValue + "' AND CPOD_I_CODE='" + ddlItemCode.SelectedValue + "' AND CPOD_I_CODE=CL_I_CODE AND CL_P_CODE='" + ddlCustomer.SelectedValue + "' AND  CL_DOC_TYPE='IWIFP' AND CPOD_CPOM_CODE=CPOM_CODE AND CL_P_CODE=CPOM_P_CODE AND IWM_CODE=CL_DOC_ID AND IWM_CODE=IWD_IWM_CODE AND IWD_CPOM_CODE=CPOD_CPOM_CODE AND IWM_INWARD_TYPE=1 ");
                    dtInvQty = CommonClasses.Execute("SELECT ISNULL(SUM(IND_INQTY),0) AS IND_INQTY FROM INVOICE_MASTER,INVOICE_DETAIL WHERE INM_CODE=IND_INM_CODE AND  INVOICE_MASTER.ES_DELETE=0 AND INM_TYPE='OutJWINM' AND IND_I_CODE=" + ddlItemCode.SelectedValue + " and IND_CPOM_CODE=" + ddlPONo.SelectedValue + " ");
                    if (dtInvQty.Rows.Count > 0)
                    {
                        inmQTy = Convert.ToDouble(dtInvQty.Rows[0]["IND_INQTY"].ToString());
                ...
                {
                    if (Convert.ToDouble(dtQty.Rows[0]["Qty"]) == 0)
                    {
                        txtPendingQty.Text = string.Format("{0:0.000}", 0);
                    }
                    else
                    {
                        txtPendingQty.Text = string.Format("{0:0.000}", Convert.ToDouble(dtQty.Rows[0]["Qty"]) - inmQTy);
                    }
                    txtVQty.Text = "0";
                }
                else
    ...
    protected void ddlItemName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (ddlItemName.SelectedIndex != 0)
            {
                ...
                {
                    dtPO = CommonClasses.Execute("SELECT DISTINCT(CPOM_CODE),CPOM_PONO FROM CUSTPO_MASTER,CUSTPO_DETAIL WHERE CPOM_CODE=CPOD_CPOM_CODE and CPOD_I_CODE='" + ddlItemCode.SelectedValue + "' AND CPOM_P_CODE='" + ddlCustomer.SelectedValue + "' AND ES_DELETE=0 and   ((CPOD_ORD_QTY-CPOD_DISPACH)>0 OR CPOD_ORD_QTY=0 AND CPOM_AUTH_FLG=1 ) --and CPOM_TYPE=-2147483648");
                }
                ddlPONo.DataSource = dtPO;
                ddlPONo.DataTextField = "CPOM_PONO";
                ddlPONo.DataValueField = "CPOM_CODE";
    ...
    private void ViewRec(string str)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dtMast.Rows[0]["INM_INV_TYPE"].ToString().ToUpper() == "TRUE")
                {

                    rbtGroup.SelectedValue = "1";//Challan
                }
                else
                {
                    rbtGroup.SelectedValue = "0";//Invoice
                }
    ...
    bool SaveRec()
    {
        ...
        try
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                {
                    //Invoice
                    dt = CommonClasses.Execute("Select isnull(max(INM_NO),0) as INM_NO FROM INVOICE_MASTER WHERE INM_CM_CODE = " + (string)Session["CompanyCode"] + " AND INM_INVOICE_TYPE=0 AND INM_INV_TYPE=0 AND INM_TYPE='OutJWINM' and ES_DELETE=0");
                }
                else
                {
                    //Challan
                    prefix = "JPDC";
                    dt = CommonClasses.Execute("Select isnull(max(INM_NO),0) as INM_NO FROM INVOICE_MASTER WHERE INM_CM_CODE = " + (string)Session["CompanyCode"] + " AND INM_INVOICE_TYPE=0 AND INM_INV_TYPE=1 AND INM_TYPE='OutJWINM' and ES_DELETE=0");
                }
                ...
                if (CommonClasses.Execute1("INSERT INTO INVOICE_MASTER (INM_CM_CODE,INM_NO,INM_DATE,INM_INVOICE_TYPE,INM_P_CODE,INM_NET_AMT,INM_DISC,INM_DISC_AMT,INM_BEXCISE,INM_BE_AMT,INM_EDUC_CESS,INM_EDUC_AMT,INM_H_EDUC_CESS,INM_H_EDUC_AMT,INM_S_TAX,INM_S_TAX_AMT,INM_TAX_TCS,INM_TAX_TCS_AMT,INM_PACK_AMT,INM_G_AMT,INM_T_CODE,INM_STO_LOC,INM_VEH_NO,INM_TRANSPORT,INM_ISSUE_DATE,INM_REMOVAL_DATE,INM_REMARK,INM_LR_NO,INM_LR_DATE,INM_ACCESSIBLE_AMT,INM_TAXABLE_AMT,INM_ROUNDING_AMT,INM_OTHER_AMT,INM_FREIGHT,INM_INSURANCE,INM_TRANS_AMT,INM_OCTRI_AMT,INM_IS_SUPPLIMENT,INM_ISSU_TIME,INM_REMOVEL_TIME,INM_TRAY_CODE,INM_TRAY_QTY,INM_TYPE,INM_TNO,INM_INV_TYPE)VALUES('" + Convert.ToInt32(Session["CompanyCode"]) + "','" + Inv_No + "','" + Convert.ToDateTime(txtDate.Text).ToString("dd/MMM/yyyy") + "','0','" + ddlCustomer.SelectedValue + "','" + txtNetAmount.Text + "','" + txtDiscPer.Text + "','" + txtDiscAmt.Text + "','" + txtBasicExcPer.Text + "','" + txtBasicExcAmt.Text + "','" + txtducexcper.Text + "','" + txtEdueceAmt.Text + "','" + txtSHEExcPer.Text + "','" + txtSHEExcAmt.Text + "','" + txtSalesTaxPer.Text + "','" + txtSalesTaxAmount.Text + "','0','" + txtTCSAmt.Text + "','" + txtPackAmt.Text + "','" + txtGrandAmt.Text + "','" + ddlTaxName.SelectedValue + "','" + txtstroreloc.Text + "','" + txtVechicleNo.Text.Trim().Replace("'", "\''") + "','" + txtTransport.Text.Trim().Replace("'", "\''") + "','" + Convert.ToDateTime(txtIssuedate.Text).ToString("dd/MMM/yyyy") + "','" + Convert.ToDateTime(txtremovaldate.Text).ToString("dd/MMM/yyyy") + "','" + txtRemark.Text.Trim().Replace("'", "\''") + "','" + txtLRNo.Text + "','" + Convert.ToDateTime(txtLRDate.Text).ToString("dd/MMM/yyyy") + "','" + txtAccessableAmt.Text + "','" + txtTaxableAmt.Text + "','" + txtRoundingAmt.Text + "','" + txtOtherCharges.Text + "','" + txtFreight.Text + "','" + txtIncurance.Text + "','" + txtTransportAmt.Text + "','" + txtOctri.Text + "','" + IsSuppliement + "','" + txtIssuetime.Text + "','" + txtRemoveltime.Text + "','" + ddlTray.SelectedValue + "','" + txtTrayQty.Text + "','OutJWINM','" + Invoice_No + "'," + rbtGroup.SelectedValue + ")"))
                {
                    ...
                    {
                        result = CommonClasses.Execute1("INSERT INTO INVOICE_DETAIL(IND_INM_CODE,IND_I_CODE,IND_CPOM_CODE,IND_INQTY,IND_RATE,IND_AMT,IND_SUBHEADING,IND_BACHNO,IND_NO_PACK,IND_PAK_QTY,IND_EX_AMT,IND_E_CESS_AMT,IND_SH_CESS_AMT,IND_REFUNDABLE_QTY,IND_PACK_DESC,IND_AMORTRATE,IND_AMORTAMT,IND_E_CODE,IND_E_TARIFF_NO) values ('" + Code + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_I_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["PO_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["INV_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["RATE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_SUBHEADING"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_BACHNO"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_NO_PACK"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_PAK_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_EX_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_CESS_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_SH_CESS_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["INV_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_PACK_DESC"].ToString().Trim().Replace("'", "\''") + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AmortRate"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AmortAmount"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_TARIFF_NO"] + "')");
                        ...
                        if (!chkIsSuppliement.Checked)
                        {
                            ...
                            int IWD_I_CODE1 = Convert.ToInt32(((DataTable)ViewState["dt2"]).Rows[i]["IND_I_CODE"].ToString());
                            if (dtchallan.Rows.Count > 0)
                            {
                                if (ddlInvoiceType.SelectedValue == "0")
                                {
                                    ...
                                    {
                                        IWD_REV_QTY = Math.Round((IWD_REV_QTY * Convert.ToDouble(rowmateril.Rows[s]["BD_QTY"].ToString())), 2);
                                        ...
                                        for (int z = 0; z < dtchallan.Rows.Count; z++)
                                        {
                                            if (Bal_Qty > 0 && dtchallan.Rows[z]["CL_I_CODE"].ToString() == IWD_I_CODE.ToString())
                                            {
                                                ...
                                                {
                                                    Bal_Qty = Math.Round((IWD_REV_QTY - Convert.ToDouble(dtchallan.Rows[z]["PENDING_QTY"].ToString())), 2);
                ...
                if (CommonClasses.Execute1("UPDATE INVOICE_MASTER SET INM_CM_CODE=" + Session["CompanyCode"] + ",INM_NO='" + txtInvoiceNo.Text + "', INM_DATE='" + Convert.ToDateTime(txtDate.Text).ToString("dd/MMM/yyyy") + "',INM_P_CODE='" + ddlCustomer.SelectedValue + "',INM_NET_AMT='" + txtNetAmount.Text + "',INM_DISC='" + txtDiscPer.Text + "',INM_DISC_AMT='" + txtDiscAmt.Text + "',INM_BEXCISE='" + txtBasicExcPer.Text + "',INM_BE_AMT='" + txtBasicExcAmt.Text + "',INM_EDUC_CESS='" + txtducexcper.Text + "',INM_EDUC_AMT='" + txtEdueceAmt.Text + "',INM_H_EDUC_CESS='" + txtSHEExcPer.Text + "',INM_H_EDUC_AMT = '" + txtSHEExcAmt.Text + "',INM_S_TAX='" + txtSalesTaxPer.Text + "',INM_S_TAX_AMT='" + txtSalesTaxAmount.Text + "',INM_PACK_AMT='" + txtPackAmt.Text + "',INM_G_AMT='" + txtGrandAmt.Text + "',INM_T_CODE='" + ddlTaxName.SelectedValue + "',INM_STO_LOC='" + txtstroreloc.Text + "',INM_VEH_NO='" + txtVechicleNo.Text.Trim().Replace("'", "\''") + "',INM_TRANSPORT='" + txtTransport.Text.Trim().Replace("'", "\''") + "',INM_ISSUE_DATE='" + Convert.ToDateTime(txtIssuedate.Text).ToString("dd/MMM/yyyy") + "',INM_REMOVAL_DATE='" + Convert.ToDateTime(txtremovaldate.Text).ToString("dd/MMM/yyyy") + "',INM_REMARK='" + txtRemark.Text + "',INM_LR_NO='" + txtLRNo.Text + "',INM_LR_DATE='" + Convert.ToDateTime(txtLRDate.Text).ToString("dd/MMM/yyyy") + "',INM_ACCESSIBLE_AMT='" + txtAccessableAmt.Text + "',INM_TAXABLE_AMT='" + txtTaxableAmt.Text + "',INM_ROUNDING_AMT='" + txtRoundingAmt.Text + "',INM_OTHER_AMT='" + txtOtherCharges.Text + "',INM_FREIGHT='" + txtFreight.Text + "',INM_INSURANCE='" + txtIncurance.Text + "',INM_TRANS_AMT='" + txtTransportAmt.Text + "',INM_OCTRI_AMT='" + txtOctri.Text + "',INM_IS_SUPPLIMENT='" + IsSuppliement + "',INM_ISSU_TIME='" + txtIssuetime.Text + "',INM_REMOVEL_TIME='" + txtRemoveltime.Text + "',INM_INV_TYPE=" + rbtGroup.SelectedValue + " where INM_CODE='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "'"))
                {
                    ...
                    if (!chkIsSuppliement.Checked)
                    {
                        for (int i = 0; i < dtq.Rows.Count; i++)
                        {
                            ...
                            if (ddlInvoiceType.SelectedValue == "0")
                            {
                                stockLedger = CommonClasses.Execute("SELECT *  FROM CHALLAN_STOCK_LEDGER where CL_DOC_ID='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and CL_DOC_TYPE='IWIAP' AND  CL_I_CODE IN (SELECT BD_I_CODE FROM BOM_MASTER,BOM_DETAIL WHERE BOM_MASTER.ES_DELETE=0 AND BM_CODE=BD_BM_CODE AND BM_I_CODE='" + dtq.Rows[i]["IND_I_CODE"] + "') ");
                                for (int z = 0; z < stockLedger.Rows.Count; z++)
                                {
                                    CommonClasses.Execute(" UPDATE CHALLAN_STOCK_LEDGER SET CL_CON_QTY =CL_CON_QTY-'" + Math.Abs(Convert.ToDouble(stockLedger.Rows[z]["CL_CQTY"].ToString())) + "' where CL_CH_NO='" + stockLedger.Rows[z]["CL_CH_NO"].ToString() + "' AND CL_DATE='" + Convert.ToDateTime(stockLedger.Rows[z]["CL_DATE"].ToString()).ToString("dd/MMM/yyyy") + "' AND CL_P_CODE='" + ddlCustomer.SelectedValue + "' AND CL_I_CODE='" + stockLedger.Rows[z]["CL_I_CODE"].ToString() + "' AND CL_DOC_TYPE='IWIAP'");
                                    CommonClasses.Execute("DELETE FROM CHALLAN_STOCK_LEDGER where CL_DOC_ID='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and CL_DOC_TYPE='IWIAP' AND CL_I_CODE='" + stockLedger.Rows[z]["CL_I_CODE"].ToString() + "'  ");
                                }
                            }
                            else
                            {
                                stockLedger = CommonClasses.Execute("SELECT *  FROM CHALLAN_STOCK_LEDGER where CL_DOC_ID='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and CL_DOC_TYPE='IWIAP' AND  CL_I_CODE='" + dtq.Rows[i]["IND_I_CODE"] + "' ");
                                for (int z = 0; z < stockLedger.Rows.Count; z++)
                                {
                                    CommonClasses.Execute(" UPDATE CHALLAN_STOCK_LEDGER SET CL_CON_QTY =CL_CON_QTY-'" + Math.Abs(Convert.ToDouble(stockLedger.Rows[z]["CL_CQTY"].ToString())) + "' where CL_CH_NO='" + stockLedger.Rows[z]["CL_CH_NO"].ToString() + "' AND CL_DATE='" + Convert.ToDateTime(stockLedger.Rows[z]["CL_DATE"].ToString()).ToString("dd/MMM/yyyy") + "' AND CL_P_CODE='" + ddlCustomer.SelectedValue + "' AND CL_I_CODE='" + stockLedger.Rows[z]["CL_I_CODE"].ToString() + "' AND CL_DOC_TYPE='IWIAP'");
                                }
                                CommonClasses.Execute("DELETE FROM CHALLAN_STOCK_LEDGER where CL_DOC_ID='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and CL_DOC_TYPE='IWIAP' AND CL_I_CODE='" + dtq.Rows[i]["IND_I_CODE"] + "'  ");
                            }
                        }
                    ...
                    {
                        // Tray Stock Entry
                        CommonClasses.Execute1("INSERT INTO STOCK_LEDGER (STL_I_CODE,STL_DOC_NO,STL_DOC_NUMBER,STL_DOC_TYPE,STL_DOC_DATE,STL_DOC_QTY,STL_STORE_TYPE) VALUES ('" + ddlTray.SelectedValue + "','" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "','" + txtInvoiceNo.Text + "','OutJWINM','" + Convert.ToDateTime(txtDate.Text).ToString("dd/MMM/yyyy") + "','-" + txtTrayQty + "','-2147483642')");
                        CommonClasses.Execute1("UPDATE ITEM_MASTER SET I_CURRENT_BAL=I_CURRENT_BAL-" + txtTrayQty.Text + ",I_ISSUE_DATE='" + Convert.ToDateTime(txtDate.Text).ToString("dd/MMM/yyyy") + "' WHERE I_CODE='" + ddlTray.SelectedValue + "'");

                        for (int i = 0; i < ((DataTable)ViewState["dt2"]).Rows.Count; i++)
                        {
                            result = CommonClasses.Execute1("INSERT INTO INVOICE_DETAIL (IND_INM_CODE,IND_I_CODE,IND_CPOM_CODE,IND_INQTY,IND_RATE,IND_AMT,IND_SUBHEADING,IND_BACHNO,IND_NO_PACK,IND_PAK_QTY,IND_EX_AMT,IND_E_CESS_AMT,IND_SH_CESS_AMT,IND_REFUNDABLE_QTY,IND_PACK_DESC,IND_AMORTRATE,IND_AMORTAMT,IND_E_CODE,IND_E_TARIFF_NO) values ('" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_I_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["PO_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["INV_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["RATE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_SUBHEADING"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_BACHNO"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_NO_PACK"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_PAK_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_EX_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_CESS_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_SH_CESS_AMT"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["INV_QTY"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_PACK_DESC"].ToString().Trim().Replace("'", "\'") + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AmortRate"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["AmortAmount"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_CODE"] + "','" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_E_TARIFF_NO"] + "')");
                            ...
                            if (!chkIsSuppliement.Checked)
                            {
                                if (result == true)
                                {
                                    ...
                                    if (ddlInvoiceType.SelectedValue == "0")
                                    {
                                        dtchallan = CommonClasses.Execute("SELECT ISNULL(SUM(CL_CQTY-CL_CON_QTY),0) as PENDING_QTY,CL_CH_NO,CL_DATE,CL_CODE,BD_VQTY+  BD_SCRAPQTY  AS BD_QTY,CL_I_CODE FROM SUPP_PO_DETAILS,BOM_MASTER,BOM_DETAIL,CHALLAN_STOCK_LEDGER where CPOD_CPOM_CODE='" + ((DataTable)ViewState["dt2"]).Rows[i]["PO_CODE"] + "' AND BM_I_CODE='" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_I_CODE"] + "' AND CPOD_I_CODE=BM_I_CODE AND BM_CODE=BD_BM_CODE AND BD_I_CODE=CL_I_CODE AND CL_P_CODE='" + ddlCustomer.SelectedValue + "'  AND CL_DOC_TYPE='IWIFP'  GROUP BY CL_CH_NO,CL_DATE,CL_CODE ,BD_VQTY, BD_SCRAPQTY ,CL_I_CODE  HAVING  SUM(CL_CQTY-CL_CON_QTY)>0 ");
                                        rowmateril = CommonClasses.Execute("SELECT BD_SCRAPQTY+BD_VQTY AS BD_QTY,BD_I_CODE FROM BOM_MASTER,BOM_DETAIL WHERE BM_CODE=BD_BM_CODE AND BM_I_CODE='" + ((DataTable)ViewState["dt2"]).Rows[i]["IND_I_CODE"] + "' AND BOM_MASTER.ES_DELETE=0");
                                    }
                                    else
                                    {
