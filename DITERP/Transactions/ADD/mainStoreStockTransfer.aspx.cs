...
public partial class Transactions_ADD_mainStoreStockTransfer : System.Web.UI.Page
{
    #region Variable
    ...
    {
        DataTable dtStore = CommonClasses.Execute("SELECT DISTINCT STORE_NAME,STORE_MASTER.STORE_CODE  FROM STORE_MASTER,USER_STORE_OWNER where USER_STORE_OWNER.STORE_CODE=STORE_MASTER.STORE_CODE  AND UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "'  AND ES_DELETE=0 and STORE_COMP_ID=" + (string)Session["CompanyId"] + "  AND STORE_MASTER.STORE_CODE  IN ( -2147483634,-2147483647) ORDER BY STORE_NAME ");
        ...
        DataTable dtStoreTo = CommonClasses.Execute("SELECT DISTINCT STORE_NAME,STORE_MASTER.STORE_CODE  FROM STORE_MASTER,USER_STORE_DETAIL where USER_STORE_DETAIL.STORE_CODE=STORE_MASTER.STORE_CODE  AND UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "'  AND ES_DELETE=0 and STORE_COMP_ID=" + (string)Session["CompanyId"] + "  AND STORE_MASTER.STORE_CODE  IN (-2147483634,-2147483647) ORDER BY STORE_NAME ");
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtRequiredQty.Text == "" || txtRequiredQty.Text == "0.000" || txtRequiredQty.Text == "0")
            {
                PanelMsg.Visible = true;
            ...
            #region Validation for Duplicate item exist in Viewstate table
            //try
            //{
            //    DataColumn[] keyColumns = new DataColumn[1];
            //    keyColumns[0] = ((DataTable)ViewState["dt2"]).Columns["ItemCode"];
            //    ((DataTable)ViewState["dt2"]).PrimaryKey = keyColumns;
            //}
            //catch (Exception Ex)
            //{
            //    //If already exist duplicate records then throw kthe msg from here
            //    PanelMsg.Visible = true;
            //    lblmsg.Text = "Duplicate Items Exists In table";// Ex.Message;
            //    ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
            //    return;
            //}
            //If new Item duplicate then retun msg from here
            //string find = "" + dr["ItemCode"] + "";
            //if (((DataTable)ViewState["dt2"]).Rows.Contains(find))
            //{
            //    PanelMsg.Visible = true;
            //    lblmsg.Text = "Duplicate Items Exists In table ";
            //    ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
            //    return;
            //}
            #endregion
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (dgIssueTo.Rows.Count != 0)
        {
            if (dgIssueTo.Enabled == true)
            {
                ...
                DataTable distinctTable = ((DataTable)ViewState["dt2"]).DefaultView.ToTable();
                DataRow[] dr = distinctTable.Select("count(ItemCode)> 1");
    ...
    private void AvoidDuplicate()
    {
        ...
        for (int i = 0; i < dgIssueTo.Rows.Count; i++)
        {
            Label txtoldvalue = dgIssueTo.Rows[i].FindControl("lblItemCode") as Label;
            string oldvalue = txtoldvalue.Text.ToString();

            if (oldvalue != "")
            {
                for (int j = i + 1; j < dgIssueTo.Rows.Count; j++)
                {
                    Label txtnewvalue = dgIssueTo.Rows[j].FindControl("lblItemCode") as Label;
                    string newvalue = txtnewvalue.Text.ToString();

                    if (oldvalue != newvalue)
                    {
                        dgIssueTo.Rows[i].Visible = true;
                    }
                    else
                    {
                        PanelMsg.Visible = true;
                        lblmsg.Text = "Duplicate records exits in Table, Please check";
                        return;
                        // dgIssueTo.Rows[i].Visible = false;
                        //break;
                    }
                }
            }
    ...
    bool SaveRec()
    {
        ...
        try
        {
            ...
            DataTable uniq_Cols = distinctTable.DefaultView.ToTable(true, "ItemCode");

            if (distinctTable.Rows.Count != uniq_Cols.Rows.Count)
            {
                PanelMsg.Visible = true;
                lblmsg.Text = "Duplicate Items Exists In table";// Ex.Message;
                ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                if (CommonClasses.Execute1("INSERT INTO ISSUE_MASTER (IM_COMP_ID,IM_NO,IM_DATE,IM_TYPE,IM_MATERIAL_REQ,IM_ISSUEBY,IM_REQBY,IM_UM_CODE,IM_FROM_STORE,IM_TRANS_TYPE)VALUES('" + Convert.ToInt32(Session["CompanyCode"]) + "','" + Po_Doc_no + "','" + Convert.ToDateTime(txtIssueDate.Text).ToString("dd/MMM/yyyy HH:mm") + "','" + ddlIssueType.SelectedValue + "','" + ddlMaterialReq.SelectedValue + "','" + txtIssueBy.Text + "','" + txtReqPerson.Text + "','" + Convert.ToInt32(Session["UserCode"].ToString()) + "','" + ddlStoreType.SelectedValue + "',1)"))
                {
                    ...
                    for (int i = 0; i < dgIssueTo.Rows.Count; i++)
                    {
                        ...
                        {
                            //If already exist duplicate records then throw the msg from here
                            PanelMsg.Visible = true;
                            lblmsg.Text = "Duplicate Items Exists In table";// Ex.Message;
                            ScriptManager.RegisterStartupScript(this, GetType(), "displayalertmessage", "Showalert();", true);
                            return result;
                        ...
                        string ToStore = "Transfer " + ddlStoreType.SelectedItem.ToString() + " - " + ((Label)dgIssueTo.Rows[i].FindControl("lblIMD_To_STORE")).Text.ToString();
                    ...
                    CommonClasses.WriteLog("Stock Transfer from Main store", "Save", "Stock Transfer from Main store", Convert.ToString(Po_Doc_no), Convert.ToInt32(Code), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                    result = true;
                    ((DataTable)ViewState["dt2"]).Rows.Clear();
            ...
            else if (Request.QueryString[0].Equals("MODIFY"))
            {
                if (CommonClasses.Execute1("UPDATE ISSUE_MASTER SET IM_TYPE='" + ddlIssueType.SelectedValue + "',IM_MATERIAL_REQ='" + ddlMaterialReq.SelectedValue + "',IM_DATE='" + Convert.ToDateTime(txtIssueDate.Text).ToString("dd/MMM/yyyy HH:mm") + "' ,IM_ISSUEBY='" + txtIssueBy.Text.Trim() + "' ,IM_REQBY='" + txtReqPerson.Text.Trim() + "' ,IM_UM_CODE='" + Convert.ToInt32(Session["UserCode"].ToString()) + "'  where IM_CODE='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and IM_COMP_ID='" + Convert.ToInt32(Session["CompanyCode"]) + "'"))
                {
                    ...
                    {
                        DataColumn[] keyColumns = new DataColumn[1];
                        //Set Primary Key of viewstate table 
                        keyColumns[0] = ((DataTable)ViewState["dt2"]).Columns["ItemCode"];
                        ((DataTable)ViewState["dt2"]).PrimaryKey = keyColumns;
                    }
                    catch (Exception Ex)
                    ...
                    result = CommonClasses.Execute1("DELETE FROM STOCK_LEDGER WHERE STL_DOC_NO='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' and STL_DOC_TYPE LIKE 'Transfer%'");
                    ...
                    if (result)
                    {
                        for (int i = 0; i < dgIssueTo.Rows.Count; i++)
                        {
                            ...
                            string find = "" + ((Label)dgIssueTo.Rows[i].FindControl("lblItemCode")).Text + "";
                        ...
                        CommonClasses.RemoveModifyLock("ISSUE_MASTER", "MODIFY", "IM_CODE", Convert.ToInt32(ViewState["mlCode"].ToString()));
                        CommonClasses.WriteLog("Stock Transfer from Main store", "Update", "Stock Transfer from Main store", txtIssueNo.Text, Convert.ToInt32(ViewState["mlCode"].ToString()), Convert.ToInt32(Session["CompanyId"]), Convert.ToInt32(Session["CompanyCode"]), (Session["Username"].ToString()), Convert.ToInt32(Session["UserCode"]));
                        ((DataTable)ViewState["dt2"]).Rows.Clear();
                        result = true;
                    }
                    Response.Redirect("~/Transactions/VIEW/mainStoreStockTransfer.aspx", false);
    ...
    protected void ddlStoreType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        BlankGridView();
        ddlItemCode_SelectedIndexChanged(null, null);
        DataTable dtStoreTo = CommonClasses.Execute("SELECT DISTINCT STORE_NAME,STORE_MASTER.STORE_CODE FROM STORE_MASTER,USER_STORE_DETAIL where USER_STORE_DETAIL.STORE_CODE=STORE_MASTER.STORE_CODE  AND UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "'  AND STORE_MASTER.STORE_CODE<>'" + ddlStoreType.SelectedValue + "' AND ES_DELETE=0 and STORE_COMP_ID=" + (string)Session["CompanyId"] + "  AND STORE_MASTER.STORE_CODE  IN ( -2147483634,-2147483647)  ORDER BY STORE_NAME ");
        ddlTostore.DataSource = dtStoreTo;
        ddlTostore.DataTextField = "STORE_NAME";
        ddlTostore.DataValueField = "STORE_CODE";
        ddlTostore.DataBind();
