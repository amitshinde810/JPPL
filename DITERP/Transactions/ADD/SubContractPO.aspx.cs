...
public partial class Transactions_ADD_SubContractPO : System.Web.UI.Page
{
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Convert.ToDouble(txtOrderQty.Text) < Convert.ToDouble(txtdispach.Text))
            {
                PanelMsg.Visible = true;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                DataTable dtPartyStae = CommonClasses.Execute("SELECT P_SM_CODE,ISNULL(P_LBT_IND,0) AS P_LBT_IND FROM PARTY_MASTER WHERE P_CODE='" + ddlSupplier.SelectedValue + "'");

                if (dtPartyStae.Rows[0]["P_LBT_IND"].ToString() == "0" || dtPartyStae.Rows[0]["P_LBT_IND"].ToString().ToUpper() == "TRUE")
                {
            ...
            fileName1 = ViewState["fileName1"].ToString();
            dr["DocName"] = ViewState["fileName"].ToString();
            dr["SPOD_E_CODE"] = ddlTariff.SelectedValue;
            dr["SPOD_E_TARIFF_NO"] = ddlTariff.SelectedItem.ToString();
    ...
    private void LoadSupplier()
    {
        try
        {
            dt = CommonClasses.Execute("select distinct(P_CODE) ,P_NAME from PARTY_MASTER where ES_DELETE=0 and P_CM_COMP_ID=" + (string)Session["CompanyId"] + " and P_TYPE='2' and P_ACTIVE_IND=1  AND (P_STM_CODE=-2147483647  OR P_STM_CODE=-2147483646)  order by P_NAME");
    ...
    #region ViewRec
    private void ViewRec(string strType)
    {
        try
        {
            LoadRateUOM();
            ...
            else if (strType == "MOD" || strType == "AMEND")
            {
    ...
    protected void dgSupplierPurchaseOrder_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "Modify")
            {
                ...
                if (Convert.ToDouble(txtConversionRetio.Text) != 0)
                {
        ...
        try
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                ...
                if (CommonClasses.Execute1("INSERT INTO SUPP_PO_MASTER (SPOM_CM_CODE,SPOM_AMOUNT,SPOM_CM_COMP_ID,SPOM_TYPE,SPOM_PO_NO,SPOM_DATE,SPOM_P_CODE,SPOM_SUP_REF,SPOM_SUP_REF_DATE,SPOM_DEL_SHCEDULE,SPOM_TRANSPOTER,SPOM_PAY_TERM1,SOM_FREIGHT_TERM,SPOM_GUARNTY,SPOM_NOTES,SPOM_DELIVERED_TO,SPOM_CURR_CODE,SPOM_CIF_NO,SPOM_PACKING,SPOM_PROJECT,SPOM_VALID_DATE,SPOM_USER_CODE,SPOM_POTYPE, SPOM_PONO,SPOM_PROJ_NAME,SPOM_IS_QUOTATION,SPOM_IS_COSTBREAKUP,SPOM_IS_AGGREMENT,SPOM_QUOTATION_FILE,SPOM_COSTBREAKUP_FILE,SPOM_AGGREMENT_FILE,SPOM_IN_NO,SPOM_PLANT)VALUES('" + Convert.ToInt32(Session["CompanyCode"]) + "','" + Convert.ToDouble(txtFinalTotalAmount.Text) + "','" + Convert.ToInt32(Session["CompanyId"]) + "', '" + ddlPOType.SelectedValue + "' ,'" + Po_Doc_no + "','" + Convert.ToDateTime(txtPoDate.Text).ToString("dd/MMM/yyyy") + "','" + ddlSupplier.SelectedValue + "','" + txtSupplierRef.Text + "','" + Convert.ToDateTime(txtSupplierRefDate.Text).ToString("dd/MMM/yyyy") + "','" + txtDeliverySchedule.Text + "','" + txtTranspoter.Text + "','" + txtPaymentTerm.Text + "','" + txtFreightTermsg.Text + "','" + txtGuranteeWaranty.Text + "','" + txtNote.Text + "','" + txtDeliveryTo.Text + "','" + ddlCurrency.SelectedValue + "','" + txtCIF.Text + "','" + txtPacking.Text + "','" + ddlProjectCode.SelectedValue + "','" + Convert.ToDateTime(txtValid.Text).ToString("dd/MMM/yyyy") + "','" + Convert.ToInt32(Session["UserCode"].ToString()) + "',1,'" + strPONo + "','" + txtProjName.Text.ToUpper().Trim() + "','" + chkIsQuotation.Checked + "','" + chkISCostBreakup.Checked + "','" + chkAggrement.Checked + "','" + ViewState["fileName1"].ToString() + "','" + ViewState["fileName2"].ToString() + "','" + ViewState["fileName3"].ToString() + "','" + ddlindentNo.SelectedValue + "','" + rbtWithAmt.SelectedValue + "')"))
                {
                    ...
                    if (ViewState["fileName1"].ToString() != "")
                    {
                        ...
                        string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName1"].ToString());
                    ...
                    if (ViewState["fileName2"].ToString() != "")
                    {
                        ...
                        // Get the destination path
                        string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName2"].ToString());
                        DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                        System.IO.File.Move(fullFilePath, copyToPath);
                    }
                    ...
                    for (int i = 0; i < dgSupplierPurchaseOrder.Rows.Count; i++)
                    {
                        ...
                        if (ViewState["fileName4"].ToString() != "")
                        {
                            ...
                            // Get the destination path
                            string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName4"].ToString());
                            DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                            System.IO.File.Move(fullFilePath, copyToPath);
                        }
                        #endregion Drawing
                        ...
                        if (ViewState["fileName5"].ToString() != "")
                        {
                            ...
                            // Get the destination path
                            string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName5"].ToString());
                            DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                            System.IO.File.Move(fullFilePath, copyToPath);
                        }
                        #endregion PDIR
                        ...
                        if (ViewState["fileName6"].ToString() != "")
                        {
                            ...
                            // Get the destination path
                            string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName6"].ToString());
                            DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                            System.IO.File.Move(fullFilePath, copyToPath);
                        }
                        ...
                        if (ViewState["fileName7"].ToString() != "")
                        {
                            ...
                            // Get the destination path
                            string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName7"].ToString());
                            DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                            System.IO.File.Move(fullFilePath, copyToPath);
                        }
                        ...
                        if (ViewState["fileName8"].ToString() != "")
                        {
                            ...
                            // Get the destination path
                            string copyToPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + Code + "/" + ViewState["fileName8"].ToString());
                            DirectoryInfo di = new DirectoryInfo(fullFilePath1);
                            System.IO.File.Move(fullFilePath, copyToPath);
                        }
                        #endregion MISC
            ...
            else if (Request.QueryString[0].Equals("AMEND"))
            {
                ...
                if (CommonClasses.Execute1("INSERT INTO SUPP_PO_AM_MASTER select SPOM_CODE,SPOM_CM_CODE,SPOM_CM_COMP_ID,SPOM_TYPE,SPOM_LM_CODE,SPOM_QUT_NO,SPOM_CONTACT_PERSON,SPOM_CONTACT_DETAIL,SPOM_PT_CODE,SPOM_PO_NO,SPOM_DATE,SPOM_P_CODE,SPOM_DISC_PER,SPOM_DISC_AMT,SPOM_AMOUNT,SPOM_TRANSPORT_AMT,SPOM_TRANSPORT_DESC,SPOM_OCTROI_PER,SPOM_OCTROI_DESC,SPOM_DELIVERY_AT,SPOM_LOADING_PER,SPOM_LOADING_DESC,SPOM_FREIGHT_AMT,SPOM_FREIGHT_DESC,SPOM_TERM_CODE,SPOM_INW_QTY,SPOM_DET_TERMS,SPOM_SCHED_S_DATE,SPOM_SCHED_E_DATE,SPOM_PAY_TERM,SPOM_REMARK,SPOM_DELIVERY_MODE,SPOM_VALID_DATE,MODIFY,ES_DELETE,SPOM_TEN_ID,SPOM_POST,SPOM_P_AMEND_CODE,SPOM_DEL_SHCEDULE,SPOM_PAY_TERM1,SPOM_TERMS_COND,SPOM_IS_AMEND,SPOM_WITH_CASH,SPOM_PO_UNIT,SPOM_SUP_REF,SPOM_SUP_REF_DATE,SOM_FREIGHT_TERM,SPOM_GUARNTY,SPOM_NOTES,SPOM_DELIVERED_TO,SPOM_TRANSPOTER,SPOM_AUTHR_FLAG,SPOM_CURR_CODE,SPOM_CIF_NO,SPOM_PACKING,SPOM_IS_SHORT_CLOSE,SPOM_AM_COUNT,SPOM_AM_DATE,SPOM_CANCEL_PO,SPOM_PROJECT, SPOM_USER_CODE,SPOM_POTYPE,SPOM_PONO,SPOM_PROJ_NAME,SPOM_IS_QUOTATION,SPOM_IS_COSTBREAKUP,SPOM_IS_AGGREMENT,SPOM_QUOTATION_FILE,SPOM_COSTBREAKUP_FILE,SPOM_AGGREMENT_FILE,SPOM_IN_NO  from SUPP_PO_MASTER where SPOM_CODE='" + Convert.ToInt32(ViewState["mlCode"].ToString()) + "' "))
                {
                    ...
                    {
                        CommonClasses.Execute1("INSERT INTO SUPP_PO_AMD_DETAILS (SPOD_AMD_CODE,SPOD_AMD_SPOM_AM_CODE,SPOD_AMD_I_CODE,SPOD_AMD_UOM_CODE,SPOD_AMD_ORDER_QTY,SPOD_AMD_RATE,SPOD_AMD_RATE_UOM,SPOD_AMD_CONV_RATIO,SPOD_AMD_DISC_PER,SPOD_AMD_DISC_AMT,SPOD_AMD_EXC_Y_N,SPOD_AMD_T_CODE,SPOD_AMD_TOTAL_AMT,SPOD_AMD_ACTIVE_IND,SPOD_AMD_SPECIFICATION,SPOD_AMD_ITEM_NAME,SPOD_AMD_INW_QTY,SPOD_AMD_EXC_PER,SPOD_AMD_EDU_CESS_PER,SPOD_AMD_H_EDU_CESS ,AMD_AM_CODE,SPOD_PROCESS_CODE,SPOD_COSTWEIGHT,SPOD_FINISHWEIGHT,SPOD_TURNINGWEIGHT,SPOD_DISPACH,SPOD_E_CODE,SPOD_E_TARIFF_NO,SPOD_EXC_AMT,SPOD_EXC_E_AMT,SPOD_EXC_HE_AMT,SPOD_AMD_IS_DRAWING,SPOD_AMD_IS_PDIR,SPOD_AMD_IS_SAMPPLAN,SPOD_AMD_IS_TRAYLAYOUT,SPOD_AMD_IS_MISC,SPOD_AMD_DRAWING_FILE,SPOD_AMD_PDIR_FILE,SPOD_AMD_SAMPPLAN_FILE,SPOD_AMD_TRAYLAYOUT_FILE,SPOD_AMD_MISC_FILE)  values('" + dtDetail.Rows[j]["SPOD_CODE"] + "','" + dtDetail.Rows[j]["SPOD_SPOM_CODE"] + "','" + dtDetail.Rows[j]["SPOD_I_CODE"] + "','" + dtDetail.Rows[j]["SPOD_UOM_CODE"] + "','" + dtDetail.Rows[j]["SPOD_ORDER_QTY"] + "','" + dtDetail.Rows[j]["SPOD_RATE"] + "','" + dtDetail.Rows[j]["SPOD_RATE_UOM"] + "','" + dtDetail.Rows[j]["SPOD_CONV_RATIO"] + "','" + dtDetail.Rows[j]["SPOD_DISC_PER"] + "','" + dtDetail.Rows[j]["SPOD_DISC_AMT"] + "','" + dtDetail.Rows[j]["SPOD_EXC_Y_N"] + "','" + dtDetail.Rows[j]["SPOD_T_CODE"] + "','" + dtDetail.Rows[j]["SPOD_TOTAL_AMT"] + "','" + dtDetail.Rows[j]["SPOD_ACTIVE_IND"] + "','" + dtDetail.Rows[j]["SPOD_SPECIFICATION"].ToString().Replace("'", "\''") + "' ,'" + dtDetail.Rows[j]["SPOD_ITEM_NAME"] + "','" + dtDetail.Rows[j]["SPOD_INW_QTY"] + "','" + dtDetail.Rows[j]["SPOD_EXC_PER"] + "','" + dtDetail.Rows[j]["SPOD_EDU_CESS_PER"] + "','" + dtDetail.Rows[j]["SPOD_H_EDU_CESS"] + "','" + MatserCode + "','" + dtDetail.Rows[j]["SPOD_PROCESS_CODE"] + "', '" + dtDetail.Rows[j]["SPOD_COSTWEIGHT"] + "','" + dtDetail.Rows[j]["SPOD_FINISHWEIGHT"] + "','" + dtDetail.Rows[j]["SPOD_TURNINGWEIGHT"] + "','" + dtDetail.Rows[j]["SPOD_DISPACH"] + "' ," + dtDetail.Rows[j]["SPOD_E_CODE"] + "," + dtDetail.Rows[j]["SPOD_E_TARIFF_NO"] + "," + dtDetail.Rows[j]["SPOD_EXC_AMT"] + "," + dtDetail.Rows[j]["SPOD_EXC_E_AMT"] + "," + dtDetail.Rows[j]["SPOD_EXC_HE_AMT"] + ",'" + dtDetail.Rows[j]["SPOD_IS_DRAWING"] + "','" + dtDetail.Rows[j]["SPOD_IS_PDIR"] + "','" + dtDetail.Rows[j]["SPOD_IS_SAMPPLAN"] + "','" + dtDetail.Rows[j]["SPOD_IS_TRAYLAYOUT"] + "','" + dtDetail.Rows[j]["SPOD_IS_MISC"] + "','" + dtDetail.Rows[j]["SPOD_DRAWING_FILE"] + "','" + dtDetail.Rows[j]["SPOD_PDIR_FILE"] + "','" + dtDetail.Rows[j]["SPOD_SAMPPLAN_FILE"] + "','" + dtDetail.Rows[j]["SPOD_TRAYLAYOUT_FILE"] + "','" + dtDetail.Rows[j]["SPOD_MISC_FILE"] + "' )");
                    }
    ...
    #region txtConversionRetio_TextChanged
    ...
    #endregion txtConversionRetio_TextChanged

    #region ddlStockUOM_SelectedIndexChanged
    protected void ddlStockUOM_SelectedIndexChanged(object sender, EventArgs e)
    {
    }
    #endregion ddlStockUOM_SelectedIndexChanged
    ...
    void FileLoactionCreate()
    {
        ...
        string sDirPath = Server.MapPath(@"~/UpLoadPath/SubConPO/" + File_Code + "");
    ...
    protected void dgDocView_RowUpdating(Object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            string directory = Server.MapPath(@"~/UpLoadPath/SubConPO/" + File_Code + "");
    ...
    #endregion dgDocView_RowUpdating

    #region lnkDownload_Click
    protected void lnkDownload_Click(object sender, EventArgs e)
    {
    }
    #endregion lnkDownload_Click
    ...
    #endregion chkExcInclusive_CheckedChanged
    ...
    private void LoadITariff()
    {
        try
        {
            dt = CommonClasses.Execute("SELECT E_CODE,E_TARIFF_NO FROM EXCISE_TARIFF_MASTER WHERE  E_EX_TYPE=1 AND E_TALLY_GST_EXCISE=1 AND  ES_DELETE=0 AND E_CM_COMP_ID=" + (string)Session["CompanyId"] + " ORDER BY E_TARIFF_NO");
            ddlTariff.DataSource = dt;
            ddlTariff.DataTextField = "E_TARIFF_NO";
            ddlTariff.DataValueField = "E_CODE";
            ddlTariff.DataBind();
            ddlTariff.Items.Insert(0, new ListItem("SAC No", "0"));
    ...
    protected void UploadDrawing(object sender, EventArgs e)
    {
        ...
        {
            chkIsDrawingFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName4"].ToString()));
        }
        lnkDrawing.Visible = true;
    ...
    protected void UploadPDIR(object sender, EventArgs e)
    {
        ...
        {
            PDIRFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName5"].ToString()));
        }
        lnkPDIR.Visible = true;
    ...
    protected void UploadsamplingPlan(object sender, EventArgs e)
    {
        ...
        {
            samplingPlanFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName6"].ToString()));
        }
        lnksamplingPlan.Visible = true;
    ...
    protected void UploadTrayLayout(object sender, EventArgs e)
    {
        ...
        {
            TrayLayoutFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName7"].ToString()));
        }
        lnkTrayLayout.Visible = true;
    ...
    protected void UploadMISCFile(object sender, EventArgs e)
    {
        ...
        {
            MISCFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName8"].ToString()));
        }
        lnkMISC.Visible = true;
    ...
    protected void lnkDrawing_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                // filePath = lnkDrawing.Text;
                directory = "../../UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + filePath;
            }
            ModalPopDocument.Show();
            IframeViewPDF.Attributes["src"] = directory;
        }
    ...
    protected void UploadQuotation(object sender, EventArgs e)
    {
        ...
        {
            IsQuotationFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName1"].ToString()));
        }
    ...
    protected void UploadCostBreakup(object sender, EventArgs e)
    {
        ...
        {
            ISCostBreakupFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName2"].ToString()));
        }
    ...
    protected void UploadAggrement(object sender, EventArgs e)
    {
        ...
        {
            AggrementFile.SaveAs(Server.MapPath("~/UpLoadPath/SubConPO/" + ViewState["mlCode"].ToString() + "/" + ViewState["fileName3"].ToString()));
        }
    ...
    private void LoadItemCode()
    {
        try
        {
            dt = CommonClasses.Execute("select I_CODE,I_CODENO,I_NAME from ITEM_MASTER where ES_DELETE=0 and I_CM_COMP_ID=" + (string)Session["CompanyId"] + "    AND I_CODE IN ( select IND_SPECIFICATION  from INDENT_DETAIL where IND_INM_CODE='" + ddlindentNo.SelectedValue + "') ORDER BY I_CODENO");
            ddlItemCode.DataSource = dt;
            ddlItemCode.DataTextField = "I_CODENO";
            ddlItemCode.DataValueField = "I_CODE";
            ddlItemCode.DataBind();
