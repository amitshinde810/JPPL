...
public partial class Transactions_ADD_TrayDelivery_Challan : System.Web.UI.Page
{
                    ...
                    if (result)
                    {
                        for (int i = 0; i < dgMainDC.Rows.Count; i++)
                        {
                            ...
                            if (result)
                            {
                                ...
                                result = CommonClasses.Execute1("INSERT INTO STOCK_LEDGER (STL_I_CODE,STL_DOC_NO,STL_DOC_NUMBER,STL_DOC_TYPE,STL_DOC_DATE,STL_DOC_QTY,STL_STORE_TYPE) VALUES ('" + ((Label)dgMainDC.Rows[i].FindControl("lblItemCode")).Text + "','" + Convert.ToInt32(ViewState["mlCode"]) + "','" + txtChallanNumber.Text + "','DCTOUT','" + Convert.ToDateTime(txtChallanDate.Text).ToString("dd/MMM/yyyy") + "','-" + ((Label)dgMainDC.Rows[i].FindControl("lblOrderQty")).Text + "','" + ((Label)dgMainDC.Rows[i].FindControl("lblStoreCode")).Text + "')");
                            }
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            dr["Remark"] = txtRemark.Text.Trim().Replace("'", "\'");
            #endregion
