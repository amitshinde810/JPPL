...
public partial class Transactions_ADD_MaterialInward : System.Web.UI.Page
{
    ...
    private void LoadCombos()
    {
        ...
        try
        {
            ...
            {
                dt = CommonClasses.Execute("SELECT DISTINCT P_CODE,P_NAME FROM PARTY_MASTER,SUPP_PO_MASTER,SUPP_PO_DETAILS WHERE PARTY_MASTER.ES_DELETE=0 AND SPOM_P_CODE=P_CODE AND SPOM_CODE=SPOD_SPOM_CODE AND SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOM_POST=1 AND SPOM_POTYPE=0 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 AND  P_CM_COMP_ID='" + (string)Session["CompanyId"] + "'  AND  SPOM_POTYPE=0 and P_TYPE='2' and P_ACTIVE_IND=1 ORDER BY P_NAME ");//AND (SPOD_INW_QTY) < (SPOD_ORDER_QTY)
               // dt = CommonClasses.Execute("SELECT DISTINCT P_CODE,P_NAME FROM PARTY_MASTER,SUPP_PO_MASTER,SUPP_PO_DETAILS WHERE PARTY_MASTER.ES_DELETE=0 AND SPOM_P_CODE=P_CODE AND SPOM_CODE=SPOD_SPOM_CODE AND SUPP_PO_MASTER.ES_DELETE=0   AND SPOM_POST=1 AND SPOM_POTYPE=0 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 AND SPOM_CM_CODE='" + Convert.ToInt32(Session["CompanyCode"]) + "'  AND P_CM_COMP_ID='" + (string)Session["CompanyId"] + "'  AND  SPOM_POTYPE=0 and P_TYPE='2' and P_ACTIVE_IND=1 ORDER BY P_NAME ");//AND (SPOD_INW_QTY) < (SPOD_ORDER_QTY)
            }
            else
            {
                dt = CommonClasses.Execute("SELECT DISTINCT P_CODE,P_NAME FROM PARTY_MASTER,SUPP_PO_MASTER,SUPP_PO_DETAILS WHERE PARTY_MASTER.ES_DELETE=0 AND SPOM_P_CODE=P_CODE AND SPOM_CODE=SPOD_SPOM_CODE AND SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOM_POST=1 AND SPOM_POTYPE=0 AND P_CM_COMP_ID='" + (string)Session["CompanyId"] + "'  AND  SPOM_POTYPE=0 and P_TYPE='2' and P_ACTIVE_IND=1 ORDER BY P_NAME ");
                //dt = CommonClasses.Execute("SELECT DISTINCT P_CODE,P_NAME FROM PARTY_MASTER,SUPP_PO_MASTER,SUPP_PO_DETAILS WHERE PARTY_MASTER.ES_DELETE=0 AND SPOM_P_CODE=P_CODE AND SPOM_CODE=SPOD_SPOM_CODE AND SUPP_PO_MASTER.ES_DELETE=0   AND SPOM_POST=1 AND SPOM_POTYPE=0 AND P_CM_COMP_ID='" + (string)Session["CompanyId"] + "'  AND  SPOM_POTYPE=0 and P_TYPE='2' and P_ACTIVE_IND=1 ORDER BY P_NAME ");
            }
        ...
        try
        {
           // dt = CommonClasses.FillCombo("ITEM_MASTER,SUPP_PO_DETAILS,SUPP_PO_MASTER", "I_NAME", "I_CODE", "SUPP_PO_MASTER.ES_DELETE=0  AND SPOM_CODE=SPOD_SPOM_CODE and i_code=SPOD_I_CODE AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' and I_CM_COMP_ID=" + (string)Session["CompanyId"] + " AND SPOM_CM_CODE='" + Convert.ToInt32(Session["CompanyCode"]) + "' ORDER BY I_NAME", ddlItemName);//and (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0))>0
            dt = CommonClasses.FillCombo("ITEM_MASTER,SUPP_PO_DETAILS,SUPP_PO_MASTER", "I_NAME", "I_CODE", "SUPP_PO_MASTER.ES_DELETE=0  AND SPOM_CODE=SPOD_SPOM_CODE and i_code=SPOD_I_CODE   and I_CM_COMP_ID=" + (string)Session["CompanyId"] + "  ORDER BY I_NAME", ddlItemName);//and (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0))>0
            ddlItemName.Items.Insert(0, new ListItem("Please Select Item ", "0"));

            //DataTable dt1 = CommonClasses.FillCombo("ITEM_MASTER,SUPP_PO_DETAILS,SUPP_PO_MASTER", "I_CODENO", "I_CODE", "SUPP_PO_MASTER.ES_DELETE=0   AND SPOM_CODE=SPOD_SPOM_CODE and i_code=SPOD_I_CODE AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' and I_CM_COMP_ID=" + (string)Session["CompanyId"] + "  AND  SPOM_CM_CODE='" + Convert.ToInt32(Session["CompanyCode"]) + "'  ORDER BY I_CODENO", ddlItemCode);//and (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0))>0
            DataTable dt1 = CommonClasses.FillCombo("ITEM_MASTER,SUPP_PO_DETAILS,SUPP_PO_MASTER", "I_CODENO", "I_CODE", "SUPP_PO_MASTER.ES_DELETE=0   AND SPOM_CODE=SPOD_SPOM_CODE and i_code=SPOD_I_CODE   and I_CM_COMP_ID=" + (string)Session["CompanyId"] + "    ORDER BY I_CODENO", ddlItemCode);//and (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0))>0
        ...
        try
        {
            //dt = CommonClasses.Execute("select SPOM_CODE,Convert(varchar,SPOM_PO_NO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) as SPOM_PO_NO from SUPP_PO_MASTER,COMPANY_MASTER where ES_DELETE=0 and SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOM_CM_COMP_ID='" + Session["CompanyId"] + "' AND SPOM_POTYPE=0 AND SPOM_CM_CODE='" + Convert.ToInt32(Session["CompanyCode"]) + "' and SPOM_CM_CODE=CM_CODE order by SPOM_CODE desc");
            dt = CommonClasses.Execute("select SPOM_CODE,Convert(varchar,SPOM_PO_NO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) as SPOM_PO_NO from SUPP_PO_MASTER,COMPANY_MASTER where ES_DELETE=0 and SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0   AND SPOM_CM_COMP_ID='" + Session["CompanyId"] + "' AND SPOM_POTYPE=0 AND   SPOM_CM_CODE=CM_CODE order by SPOM_CODE desc");
    ...
    public bool Setvalues()
    {
        ...
        try
        {
            ...
            BL_InwardMaster.IWM_CHALLAN_NO = txtChallanNo.Text.Trim().Replace("'", "\'");
    ...
    protected void BtnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtChallanQty.Text.Trim() == "" || Convert.ToDecimal(txtChallanQty.Text) <= 0)
            {
                PanelMsg.Visible = true;
            ...
            if (txtRecdQty.Text.Trim() == "" || Convert.ToDecimal(txtRecdQty.Text) <= 0)
            {
                PanelMsg.Visible = true;
            ...
            dr["IWD_REMARK"] = txtRemark.Text.Trim().Replace("'", "\'");
            dr["I_UOM_NAME"] = ddlUom.SelectedItem;
            dr["IWD_UOM_CODE"] = ddlUom.SelectedValue;
            dr["IWD_BATCH_NO"] = txtBatchNo.Text.Trim().Replace("'", "\'");
            #endregion
    ...
    protected void ddlPoNumber_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            /*spod_order_qty=0*/
            dt = CommonClasses.Execute("select (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0)) as PENDQTY,  case when SPOD_ORDER_QTY =0 then SPOD_RATE  else SPOD_RATE - ROUND((SPOD_DISC_AMT/SPOD_ORDER_QTY),2) end AS SPOD_RATE,SPOD_UOM_CODE,SPOM_POST from SUPP_PO_MASTER,SUPP_PO_DETAILS where SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_CODE=SPOD_SPOM_CODE AND SPOM_P_CODE='" + ddlSupplier.SelectedValue + "'   AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' and SPOM_CODE='" + ddlPoNumber.SelectedValue + "'");
            if (dt.Rows.Count > 0)
            {
    ...
    protected void ddlSupplier_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            DataTable dtItem = new DataTable();
            dtItem = CommonClasses.Execute("SELECT DISTINCT I_NAME,I_CODE,I_CODENO FROM ITEM_MASTER,SUPP_PO_DETAILS,SUPP_PO_MASTER WHERE SUPP_PO_MASTER.ES_DELETE=0 and I_CODE=SPOD_I_CODE and SPOM_CANCEL_PO=0 and SPOM_POST=1  AND SPOM_IS_SHORT_CLOSE=0 AND SPOM_CODE=SPOD_SPOM_CODE and I_CM_COMP_ID='" + (string)Session["CompanyId"] + "'  AND SPOM_POTYPE=0 AND SPOM_POST=1 AND SPOM_P_CODE=" + id + " ORDER BY I_NAME");//AND (SPOD_INW_QTY) < (SPOD_ORDER_QTY)
    ...
    private void pendingQty()
    {
        ...
        try
        {
            ...
            if (Request.QueryString[0].Equals("MODIFY"))
            {
                dtPO = CommonClasses.Execute("SELECT DISTINCT SPOM_CODE,Convert(varchar,SPOM_PONO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) +' -'+ISNULL(PROCM_NAME,'') as SPOM_PO_NO FROM SUPP_PO_MASTER,SUPP_PO_DETAILS,PROJECT_CODE_MASTER,ITEM_MASTER,COMPANY_MASTER WHERE PROCM_CODE=SPOM_PROJECT AND  SPOM_CODE=SPOD_SPOM_CODE AND SPOM_POTYPE=0 AND SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 and  SPOM_P_CODE='" + ddlSupplier.SelectedValue + "'  AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOD_I_CODE=I_CODE AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "'    and SPOM_CM_CODE=CM_CODE AND  SUPP_PO_MASTER.ES_DELETE=0 and SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' order by SPOM_CODE");
                //dtPO = CommonClasses.Execute("SELECT DISTINCT SPOM_CODE,Convert(varchar,SPOM_PONO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) +' -'+ISNULL(PROCM_NAME,'') as SPOM_PO_NO FROM SUPP_PO_MASTER,SUPP_PO_DETAILS,PROJECT_CODE_MASTER,ITEM_MASTER,COMPANY_MASTER WHERE PROCM_CODE=SPOM_PROJECT AND  SPOM_CODE=SPOD_SPOM_CODE AND SPOM_POTYPE=0 AND SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 and  SPOM_P_CODE='" + ddlSupplier.SelectedValue + "'   AND SPOD_I_CODE=I_CODE AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "'  AND SPOM_CM_CODE= '" + Convert.ToInt32(Session["CompanyCode"].ToString()) + "' and SPOM_CM_CODE=CM_CODE AND  SUPP_PO_MASTER.ES_DELETE=0 and SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' order by SPOM_CODE");
            }
            else
            {
                dtPO = CommonClasses.Execute("SELECT DISTINCT SPOM_CODE,Convert(varchar,SPOM_PONO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) +' -'+ISNULL(PROCM_NAME,'') as SPOM_PO_NO FROM SUPP_PO_MASTER,SUPP_PO_DETAILS,PROJECT_CODE_MASTER,ITEM_MASTER,COMPANY_MASTER WHERE PROCM_CODE=SPOM_PROJECT AND  SPOM_CODE=SPOD_SPOM_CODE  AND SPOM_POTYPE=0 AND SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 and  SPOM_P_CODE='" + ddlSupplier.SelectedValue + "' AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOD_I_CODE=I_CODE AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "'  AND   SPOM_CM_CODE=CM_CODE AND  SUPP_PO_MASTER.ES_DELETE=0 and SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' order by SPOM_CODE");//AND (SPOD_INW_QTY) < (SPOD_ORDER_QTY)
                //dtPO = CommonClasses.Execute("SELECT DISTINCT SPOM_CODE,Convert(varchar,SPOM_PONO) + '/' + Right(Convert(varchar,Year(CM_OPENING_DATE)),2)+ '-' + Right(Convert(varchar,Year(CM_CLOSING_DATE)),2) +' -'+ISNULL(PROCM_NAME,'') as SPOM_PO_NO FROM SUPP_PO_MASTER,SUPP_PO_DETAILS,PROJECT_CODE_MASTER,ITEM_MASTER,COMPANY_MASTER WHERE PROCM_CODE=SPOM_PROJECT AND  SPOM_CODE=SPOD_SPOM_CODE  AND SPOM_POTYPE=0 AND SPOM_POST=1 AND SPOM_IS_SHORT_CLOSE=0 and SPOM_CANCEL_PO=0 and  SPOM_P_CODE='" + ddlSupplier.SelectedValue + "'   AND SPOD_I_CODE=I_CODE AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "'  AND SPOM_CM_CODE= '" + Convert.ToInt32(Session["CompanyCode"].ToString()) + "' and SPOM_CM_CODE=CM_CODE AND  SUPP_PO_MASTER.ES_DELETE=0 and SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' order by SPOM_CODE");//AND (SPOD_INW_QTY) < (SPOD_ORDER_QTY)
            }
            ...
            ddlPoNumber.DataValueField = "SPOM_CODE";
            ddlPoNumber.DataBind();
            //dt = CommonClasses.Execute("SELECT (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0)) AS PENDQTY, SPOD_RATE - ROUND((SPOD_DISC_AMT/SPOD_ORDER_QTY),2) AS SPOD_RATE,SPOD_UOM_CODE,SPOM_POST from SUPP_PO_MASTER,SUPP_PO_DETAILS where SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_CODE=SPOD_SPOM_CODE AND SPOM_P_CODE='" + ddlSupplier.SelectedValue + "' AND SPOM_POTYPE=0 AND SPOM_CM_CODE= '" + Convert.ToInt32(Session["CompanyCode"].ToString()) + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' and SPOM_CODE='" + ddlPoNumber.SelectedValue + "'");
            /* If SPOD_ORDER_QTY=0 then SPOD rate else SPOD_RATE - (SPOD_DISC_AMT/SPOD_ORDER_QTY)*/
            //dt = CommonClasses.Execute("SELECT (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0)) AS PENDQTY, case when SPOD_ORDER_QTY =0 then SPOD_RATE else SPOD_RATE - ROUND((SPOD_DISC_AMT/SPOD_ORDER_QTY),2) end AS SPOD_RATE,SPOD_UOM_CODE,SPOM_POST from SUPP_PO_MASTER,SUPP_PO_DETAILS where SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_CODE=SPOD_SPOM_CODE AND SPOM_P_CODE='" + ddlSupplier.SelectedValue + "' AND SPOM_PLANT='" + rbtWithAmt.SelectedValue + "' AND SPOM_POTYPE=0 AND SPOM_CM_CODE= '" + Convert.ToInt32(Session["CompanyCode"].ToString()) + "' AND SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' and SPOM_CODE='" + ddlPoNumber.SelectedValue + "'");
            dt = CommonClasses.Execute("SELECT (ISNULL(SPOD_ORDER_QTY,0)-ISNULL(SPOD_INW_QTY,0)) AS PENDQTY, case when SPOD_ORDER_QTY =0 then SPOD_RATE else SPOD_RATE - ROUND((SPOD_DISC_AMT/SPOD_ORDER_QTY),2) end AS SPOD_RATE,SPOD_UOM_CODE,SPOM_POST from SUPP_PO_MASTER,SUPP_PO_DETAILS where SUPP_PO_MASTER.ES_DELETE=0 AND SPOM_CODE=SPOD_SPOM_CODE AND SPOM_P_CODE='" + ddlSupplier.SelectedValue + "'  AND SPOM_POTYPE=0 AND     SPOM_CM_COMP_ID='" + Convert.ToInt32(Session["CompanyId"]) + "' AND SPOD_I_CODE='" + ddlItemName.SelectedValue + "' and SPOM_CODE='" + ddlPoNumber.SelectedValue + "'");
        ...
        if (dtApproved.Rows.Count > 0)
        {
            if (dtApproved.Rows[0]["P_INHOUSE_IND"].ToString().ToUpper() == "TRUE" && txtSchdeuleQTy.Text!="NA")
            {
                ...
                if (dtCQty.Rows.Count > 0)
                {
                    ...
                    if (Convert.ToDouble(txtSchdeuleQTy.Text) >= Convert.ToDouble(txtChallanQty.Text))
                    {
    ...
    public void challandetail()
    {
        if (ddlSupplier.SelectedValue != "0" && txtChallanNo.Text != "" && txtChallanNo.Text != "0" && txtChallanDate.Text != "")
        {
            ...
            if (Request.QueryString[0].Equals("INSERT"))
            {
                dtChallan = CommonClasses.Execute("select * from INWARD_MASTER where IWM_CHALLAN_NO='" + txtChallanNo.Text.Trim() + "' and IWM_P_CODE='" + ddlSupplier.SelectedValue + "' and ES_DELETE=0 and IWM_CM_CODE=" + Convert.ToInt32(Session["CompanyCode"]) + "  ");
            }
            else
            {
                dtChallan = CommonClasses.Execute("select * from INWARD_MASTER where IWM_CHALLAN_NO='" + txtChallanNo.Text.Trim() + "' and IWM_P_CODE='" + ddlSupplier.SelectedValue + "' and ES_DELETE=0 and IWM_CM_CODE=" + Convert.ToInt32(Session["CompanyCode"]) + "   AND IWM_CODE!='" + ViewState["mlCode"].ToString() + "'");
            }
