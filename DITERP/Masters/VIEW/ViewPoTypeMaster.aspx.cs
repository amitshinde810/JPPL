...
public partial class Masters_VIEW_ViewPoTypeMaster : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    DataTable dtRights = CommonClasses.Execute("select UR_RIGHTS from USER_RIGHT where UR_IS_DELETE=0 AND UR_UM_CODE='" + Convert.ToInt32(Session["UserCode"]) + "' and UR_SM_CODE='33'");
    ...
    protected void txtString_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Po Type Master- View", "txtString_TextChanged", Ex.Message);
        }
    }
    #endregion
    ...
    private void LoadStatus(TextBox txtString)
    {
        try
        {
            ...
            if (txtString.Text != "")
                dtfilter = CommonClasses.Execute("SELECT  * FROM PO_TYPE_MASTER WHERE ES_DELETE='FALSE' AND PO_T_COMP_ID = '" + Session["CompanyId"] + "' and (lower(PO_T_SHORT_NAME) like lower('%" + str + "%') or lower(PO_T_DESC) like lower('%" + str + "%') or lower(PO_T_FIRST_LETTER) like lower('%" + str + "%') ) order by PO_T_SHORT_NAME ");
            else
                dtfilter = CommonClasses.Execute("SELECT  * FROM PO_TYPE_MASTER WHERE ES_DELETE='FALSE' AND PO_T_COMP_ID = '" + Session["CompanyId"] + "' order by PO_T_SHORT_NAME");
        ...
        {
            CommonClasses.SendError("Po Type Master - View", "LoadStatus", Ex.Message);
        }
    }
    #endregion
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string type = "INSERT";
                Response.Redirect("~/Masters/ADD/PoTypeMaster.aspx?c_name=" + type, false);
            }
            else
            {
        ...
        {
            CommonClasses.SendError("Po Type Master", "btnInsert_Click", exc.Message);
        }
    }
    #endregion
    ...
    protected void GridView1_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Po Type Master", "GridView1_PageIndexChanging", Ex.Message);
        }
    }
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
            ...
            if (e.CommandName.Equals("View"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For View"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        Response.Redirect("~/Masters/ADD/PoTypeMaster.aspx?c_name=" + type + "&u_code=" + um_code, false);
                    }
                    else
                    {
                        PanelMsg.Visible = true;
        ...
        {
            CommonClasses.SendError("Po Type Master", "GridView1_RowCommand", exc.Message);
        }
    }
    ...
    bool ModifyLog(string PrimaryKey)
    {
        try
        {
            ...
            DataTable dt = CommonClasses.Execute("select MODIFY from PO_TYPE_MASTER where PO_T_CODE=" + PrimaryKey + "  ");
    ...
    protected void btnClose_Click(object sender, EventArgs e)
    {
        ...
        {
            CommonClasses.SendError("Po Type Master", "btnClose_Click", ex.Message);
        }
    }
