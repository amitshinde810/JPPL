...
public partial class Masters_VIEW_ViewSupplierMaster : System.Web.UI.Page
{
    ...
    private void LoadSupplier()
    {
        try
        {
            ...
            dt = CommonClasses.Execute("SELECT TOP 500 P_CODE,P_PARTY_CODE,P_NAME,P_CONTACT,P_PHONE FROM PARTY_MASTER WHERE P_CM_COMP_ID = '" + Convert.ToInt32(Session["CompanyId"]) + "' and P_TYPE=2 and ES_DELETE=0 ORDER BY P_NAME");
            if (dt.Rows.Count == 0)
            {
    ...
    private void LoadStatus(TextBox txtString)
    {
        try
        {
            ...
            if (txtString.Text != "")
                dtfilter = CommonClasses.Execute("SELECT TOP 500 P_CODE,P_PARTY_CODE,P_NAME,P_CONTACT,P_PHONE FROM PARTY_MASTER WHERE P_CM_COMP_ID = '" + Convert.ToInt32(Session["CompanyId"]) + "' and ES_DELETE='0' and P_TYPE=2 and (P_PARTY_CODE like upper('%" + str + "%') OR P_NAME like upper('%" + str + "%') OR P_CONTACT like upper('%" + str + "%') OR P_PHONE like upper('%" + str + "%')) order by P_NAME");
            else
                dtfilter = CommonClasses.Execute("SELECT TOP 500 P_CODE,P_PARTY_CODE,P_NAME,P_CONTACT,P_PHONE FROM PARTY_MASTER WHERE P_CM_COMP_ID = '" + Convert.ToInt32(Session["CompanyId"]) + "' and P_TYPE=2 and ES_DELETE='0' order by P_NAME");
