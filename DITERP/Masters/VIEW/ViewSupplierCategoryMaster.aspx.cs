...
public partial class Masters_VIEW_ViewSupplierCategoryMaster : System.Web.UI.Page
{
                ...
                if (!IsPostBack)
                {
                    ...
                    if (CommonClasses.ValidRights(int.Parse(right.Substring(0, 1)), this, "For Menu"))
                    {
                        ...
                        if (dgSuppCategory.Rows.Count == 0)
                        {
                            ...
                            {
                                dtFilter.Columns.Add(new System.Data.DataColumn("SCM_CODE", typeof(string)));
                                dtFilter.Columns.Add(new System.Data.DataColumn("SCM_NAME", typeof(string)));
    ...
    private void LoadStore()
    {
        try
        {
            ...
            dt = CommonClasses.Execute("SELECT * FROM SUPPLIER_CATEGORY_MASTER WHERE SCM_CM_COMP_ID= '" + Convert.ToInt32(Session["CompanyId"]) + "' and ES_DELETE=0 order by SCM_NAME ");
    ...
    protected void btnInsert_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string type = "INSERT";
                Response.Redirect("~/Masters/Add/SupplierCategoryMaster.aspx?c_name=" + type, false);
            }
            else
            {
    ...
    private void LoadStatus(TextBox txtString)
    {
        try
        {
            ...
            if (txtString.Text != "")
                dtfilter = CommonClasses.Execute("SELECT  * FROM SUPPLIER_CATEGORY_MASTER WHERE ES_DELETE='FALSE' AND SCM_CM_COMP_ID = '" + Session["CompanyId"] + "' and lower(SCM_NAME) like lower('%" + str + "%') order by SCM_NAME");
            else
                dtfilter = CommonClasses.Execute("SELECT  * FROM SUPPLIER_CATEGORY_MASTER WHERE ES_DELETE='FALSE' AND SCM_CM_COMP_ID = '" + Session["CompanyId"] + "' order by SCM_NAME ");
    ...
    protected void dgSuppCategory_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("View"))
            {
                if (CommonClasses.ValidRights(int.Parse(right.Substring(1, 1)), this, "For View"))
                {
                    if (!ModifyLog(e.CommandArgument.ToString()))
                    {
                        ...
                        Response.Redirect("~/Masters/Add/SupplierCategoryMaster.aspx?c_name=" + type + "&u_code=" + um_code, false);
                    }
                    else
                    {
                        PanelMsg.Visible = true;
            ...
            if (e.CommandName.Equals("Modify"))
            {
                ...
                {
                    string um_code = e.CommandArgument.ToString();
                    DataTable dtFixStores = CommonClasses.Execute("select SCM_CODE,SCM_NAME from SUPPLIER_CATEGORY_MASTER where SCM_CODE in (-2147483648,-2147483647,-2147483646,-2147483645,-2147483644,-2147483643,-2147483642,-2147483641) and SCM_CODE='" + um_code + "' AND ES_DELETE=0");
    ...
    bool ModifyLog(string PrimaryKey)
    {
        try
        {
            ...
            DataTable dt = CommonClasses.Execute("select MODIFY from SUPPLIER_CATEGORY_MASTER where SCM_CODE=" + PrimaryKey + "  ");
    ...
    protected void dgSuppCategory_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            {
                if (!ModifyLog(((Label)(dgSuppCategory.Rows[e.RowIndex].FindControl("lblSCM_CODE"))).Text))
                {
                    bool flag = false;
                    string um_code = ((Label)(dgSuppCategory.Rows[e.RowIndex].FindControl("lblSCM_CODE"))).Text;
                    string um_name = ((Label)(dgSuppCategory.Rows[e.RowIndex].FindControl("lblSCM_NAME"))).Text;
                    ...
                    {
                        flag = CommonClasses.Execute1("UPDATE SUPPLIER_CATEGORY_MASTER SET ES_DELETE = 1 WHERE SCM_CODE='" + Convert.ToInt32(um_code) + "'");
                        PanelMsg.Visible = true;
    ...
    protected void dgSuppCategory_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            ...
            {
                string user_code = ((Label)(dgSuppCategory.Rows[e.NewEditIndex].FindControl("lblSCM_CODE"))).Text;
                string type = "MODIFY";
