...
public partial class Masters_VIEW_ViewSectorMaster : System.Web.UI.Page
{
    ...
    private void LoadStatus(TextBox txtString)
    {
        try
        {
            ...
            DataTable dtfilter = new DataTable();
            if (txtString.Text != "")
                dtfilter = CommonClasses.Execute("SELECT E_CODE, E_CM_COMP_ID, E_TARIFF_NO, E_COMMODITY, E_BASIC, E_SPECIAL, E_EDU_CESS, E_H_EDU FROM  EXCISE_TARIFF_MASTER WHERE E_CM_COMP_ID='" + Session["CompanyId"] + "' and ES_DELETE='0' and (E_TARIFF_NO like upper('%" + str + "%') OR E_COMMODITY like upper('%" + str + "%') OR E_BASIC like upper('%" + str + "%') OR E_SPECIAL like upper('%" + str + "%') OR E_EDU_CESS like upper('%" + str + "%'))");
            else
                dtfilter = CommonClasses.Execute("SELECT E_CODE, E_CM_COMP_ID, E_TARIFF_NO, E_COMMODITY, E_BASIC, E_SPECIAL, E_EDU_CESS, E_H_EDU FROM  EXCISE_TARIFF_MASTER WHERE E_CM_COMP_ID='" + Session["CompanyId"] + "' and ES_DELETE='0'");
